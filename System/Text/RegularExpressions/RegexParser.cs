...
namespace System.Text.RegularExpressions
{
	...
	internal sealed class RegexParser
	{
		...
		internal static RegexTree Parse(string re, RegexOptions op)
		{
			...
			{
				capslist = (string[])regexParser._capnamelist.ToArray(typeof(string));
			}
		...
		internal void AssignNameSlots()
		{
			if (this._capnames != null)
			{
				for (int i = 0; i < this._capnamelist.Count; i++)
				{
					...
					string key = (string)this._capnamelist[i];
			...
			if (this._capcount < this._captop)
			{
				...
				{
					this._capnumlist[num++] = enumerator.Key;
				}
			...
			if (this._capnames != null || this._capnumlist != null)
			{
				...
				else
				{
					...
					this._capnamelist = new ArrayList();
					num3 = (int)this._capnames[arrayList[0]];
				}
				for (int j = 0; j < this._capcount; j++)
				{
					int num4 = (this._capnumlist == null) ? j : ((int)this._capnumlist[j]);
					if (num3 == num4)
					{
						this._capnamelist.Add((string)arrayList[num2++]);
						num3 = ((num2 == arrayList.Count) ? -1 : ((int)this._capnames[arrayList[num2]]));
					}
					else
					{
