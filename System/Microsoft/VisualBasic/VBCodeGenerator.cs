...
namespace Microsoft.VisualBasic
{
	...
	internal class VBCodeGenerator : CodeCompiler
	{
		...
		protected override string CmdArgsFromParameters(CompilerParameters options)
		{
			...
			foreach (string value in options.EmbeddedResources)
			{
				stringBuilder.Append("/res:\"");
				stringBuilder.Append(value);
				stringBuilder.Append("\" ");
			}
			foreach (string value2 in options.LinkedResources)
			{
		...
		protected override CompilerResults FromFileBatch(CompilerParameters options, string[] fileNames)
		{
			...
			base.Compile(options, RedistVersionInfo.GetCompilerPath(this.provOptions, this.CompilerName), this.CompilerName, text, ref path, ref num, trueArgs);
