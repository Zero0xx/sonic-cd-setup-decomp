...
namespace System.Diagnostics
{
	...
	internal static class AssertWrapper
	{
		...
		{
			IntPtr handle = SafeNativeMethods.GetStockObject(17);
			IntPtr handle2 = UnsafeNativeMethods.GetDC(NativeMethods.NullHandleRef);
			NativeMethods.TEXTMETRIC textmetric = new NativeMethods.TEXTMETRIC();
			handle = UnsafeNativeMethods.SelectObject(new HandleRef(null, handle2), new HandleRef(null, handle));
			SafeNativeMethods.GetTextMetrics(new HandleRef(null, handle2), textmetric);
			UnsafeNativeMethods.SelectObject(new HandleRef(null, handle2), new HandleRef(null, handle));
			UnsafeNativeMethods.ReleaseDC(NativeMethods.NullHandleRef, new HandleRef(null, handle2));
			handle2 = IntPtr.Zero;
