...
namespace System.Runtime.Serialization.Formatters.Soap
{
	...
	internal sealed class ObjectWriter
	{
		...
		internal void Serialize(object graph, Header[] inHeaders, SoapWriter serWriter)
		{
			...
			if (serWriter == null)
			{
				throw new ArgumentNullException("serWriter", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[]
				{
					"serWriter"
				}));
			}
		...
		private void ProcessHeaders(long headerId)
		{
			...
			while ((next = this.GetNext(out objectId)) != null)
			{
				...
				writeObjectInfo2.objectId = objectId;
		...
		private void Write(WriteObjectInfo objectInfo, NameInfo memberNameInfo, NameInfo typeNameInfo)
		{
			...
			if (obj == null)
			{
				throw new ArgumentNullException("objectInfo.obj", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArgumentNull_Obj"), new object[]
				{
			...
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SoapNoGenericsSupport"), new object[]
				{
					objectInfo.objectType
				}));
			}
		...
		private void WriteMembers(NameInfo memberNameInfo, NameInfo memberTypeNameInfo, object memberData, WriteObjectInfo objectInfo, NameInfo typeNameInfo, WriteObjectInfo memberObjectInfo, bool isAttribute)
		{
			...
			if (memberObjectInfo == null || !memberObjectInfo.isArray)
			{
				if (!this.WriteKnownValueClass(memberNameInfo, memberTypeNameInfo, memberData, isAttribute))
				{
					...
					if (isAttribute)
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_NonPrimitive_XmlAttribute"), new object[]
						{
		...
		private void WriteArray(WriteObjectInfo objectInfo, NameInfo memberNameInfo, WriteObjectInfo memberObjectInfo)
		{
			...
			else if (internalArrayTypeE == InternalArrayTypeE.Jagged)
			{
				...
				if (num4 < 0)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Dimensions"), new object[]
					{
						nameInfo.NIname
					}));
				}
		...
		private void WriteArrayMember(WriteObjectInfo objectInfo, NameInfo arrayElemTypeNameInfo, object data)
		{
			...
			if (!this.WriteKnownValueClass(arrayElemTypeNameInfo, nameInfo, data, false))
			{
				...
				{
					WriteObjectInfo objectInfo2 = WriteObjectInfo.Serialize(data, this.m_surrogates, this.m_context, this.serObjectInfoInit, this.m_formatterConverter, null, this);
					nameInfo.NIassemId = this.GetAssemblyId(objectInfo2);
				...
				else
				{
					...
					if (num < 1L)
					{
						...
						{
							this.WriteArray(writeObjectInfo, nameInfo, null);
						}
						else
						{
		...
		private object GetNext(out long objID)
		{
			...
			if (flag)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ObjNoID"), new object[]
				{
					obj
				}));
			}
		...
		private long Schedule(object obj, Type type, WriteObjectInfo objectInfo)
		{
			...
			bool flag;
			long result = this.InternalGetId(obj, type, out flag);
			if (flag)
			{
				if (objectInfo == null)
				{
		...
		private NameInfo TypeToNameInfo(Type type, WriteObjectInfo objectInfo, InternalPrimitiveTypeE code, NameInfo nameInfo)
		{
			...
			string niname = null;
			nameInfo.NInameSpaceEnum = Converter.GetNameSpaceEnum(code, type, objectInfo, out niname);
			nameInfo.NIprimitiveTypeEnum = code;
			nameInfo.NItype = type;
			nameInfo.NIname = niname;
			if (objectInfo != null)
			{
		...
		private void ArrayNameToDisplayName(WriteObjectInfo objectInfo, NameInfo arrayElemTypeNameInfo)
		{
			...
			if (num > 0)
			{
				...
				if (internalPrimitiveTypeE != InternalPrimitiveTypeE.Invalid)
				{
					...
					{
						flag = true;
						str = Converter.ToXmlDataType(internalPrimitiveTypeE);
						string text2 = null;
						arrayElemTypeNameInfo.NInameSpaceEnum = Converter.GetNameSpaceEnum(internalPrimitiveTypeE, null, objectInfo, out text2);
					}
				}
				...
				if (flag)
				{
					arrayElemTypeNameInfo.NIname = str + niname.Substring(num);
					return;
				}
			}
		...
		private long GetAssemblyId(WriteObjectInfo objectInfo)
		{
			...
			else if (assemblyString.Equals(Converter.urtAssemblyString))
			{
				num = 0L;
				isNew = false;
				this.serWriter.WriteAssembly(objectInfo.GetTypeFullName(), objectInfo.objectType, null, (int)num, isNew, objectInfo.IsAttributeNameSpace());
			}
			else
			{
				...
				{
					num = this.m_idGenerator.GetId("___AssemblyString___" + assemblyString, out isNew);
				...
				this.serWriter.WriteAssembly(objectInfo.GetTypeFullName(), objectInfo.objectType, assemName, (int)num, isNew, objectInfo.IsInteropNameSpace());
			}
			return num;
		}
		...
		private bool IsEmbeddedAttribute(Type type)
		{
			...
			else
			{
				...
				result = soapTypeAttribute.Embedded;
			}
			return result;
		}
