...
namespace System.Runtime.Serialization.Formatters.Soap
{
	...
	internal sealed class ObjectReader
	{
		...
		internal object Deserialize(HeaderHandler handler, ISerParser serParser)
		{
			if (serParser == null)
			{
				throw new ArgumentNullException("serParser", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[]
				{
					serParser
				}));
			}
			...
			if (!this.isTopObjectResolved)
			{
				...
				for (int i = 0; i < num; i++)
				{
					ParseRecord pr = (ParseRecord)this.topStack.Pop();
					this.Parse(pr);
				}
			}
			this.m_objectManager.DoFixups();
		...
		internal void Parse(ParseRecord pr)
		{
			...
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_XMLElement"), new object[]
			{
				pr.PRname
			}));
		}
		...
		{
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ParseError"), new object[]
			{
		...
		private void CheckSecurity(ParseRecord pr)
		{
			...
			if (prdtType != null && this.IsRemoting)
			{
				...
				{
					throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MBRAsMBV"), new object[]
					{
						prdtType.FullName
					}));
				}
		...
		private void ParseObject(ParseRecord pr)
		{
			...
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TopObjectInstantiate"), new object[]
				{
					pr.PRname
				}));
			}
			...
			else
			{
				...
				if (pr.PRdtType == null)
				{
					...
					{
						throw new ServerException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SoapFault"), new object[]
						{
							this.faultString
						}));
					}
		...
		private void ParseObjectEnd(ParseRecord pr)
		{
			...
			if (parseRecord.PRnewObj == null)
			{
				if (parseRecord.PRdtType != Converter.typeofString)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ObjectMissing"), new object[]
					{
						pr.PRname
					}));
				}
		...
		private void ParseMember(ParseRecord pr)
		{
			...
			if (pr.PRdtType == null && parseRecord.PRobjectInfo.isTyped)
			{
				...
				if (pr.PRdtType == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeResolved"), new object[]
					{
						parseRecord.PRnewObj + " " + pr.PRname
					}));
				}
		...
		internal long GetId(string keyId)
		{
			if (keyId == null)
			{
				throw new ArgumentNullException("keyId", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[]
				{
					"keyId"
				}));
			}
			if (keyId != this.inKeyId)
			{
				...
				if (keyId[0] == '#')
				{
					key = keyId.Substring(1);
				}
				else
				{
					key = keyId;
				}
				object obj = this.objectIdTable[key];
				if (obj == null)
				{
					this.outKeyId = (this.objectIds += 1L);
					this.objectIdTable[key] = this.outKeyId;
				}
				else
				{
		...
		internal Type Bind(string assemblyString, string typeString)
		{
			...
			{
				result = this.m_binder.BindToType(assemblyString, typeString);
			}
			return result;
		}
