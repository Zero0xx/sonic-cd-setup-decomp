...
namespace System.Runtime.Serialization.Formatters.Soap
{
	...
	internal sealed class SoapWriter
	{
		...
		internal SoapWriter(Stream stream)
		{
			...
			this.writer = new StreamWriter(stream, encoding, 1024);
		...
		private string Escape(string value)
		{
			...
			{
				result = this.stringBuffer.ToString();
			}
			else
			{
				result = value;
		...
		internal void Write(InternalElementTypeE use, string name, AttributeList attrList, string value, bool isNameEscape, bool isValueEscape)
		{
			...
			case InternalElementTypeE.Member:
				...
				break;
			default:
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_UseCode"), new object[]
				{
					use.ToString()
				}));
			}
		...
		private void WriteAttributeList(AttributeList attrList)
		{
			for (int i = 0; i < attrList.Count; i++)
			{
				string s;
				string s2;
				attrList.Get(i, out s, out s2);
				this.InternalWrite(" ");
				this.InternalWrite(s);
				this.InternalWrite("=");
				this.InternalWrite("\"");
				this.InternalWrite(s2);
				this.InternalWrite("\"");
			}
		}
		...
		internal void WriteObject(NameInfo nameInfo, NameInfo typeNameInfo, int numMembers, string[] memberNames, Type[] memberTypes, WriteObjectInfo[] objectInfos)
		{
			...
			if (this.attrValueList.Count > 0)
			{
				for (int i = 0; i < this.attrValueList.Count; i++)
				{
					string name;
					string value;
					this.attrValueList.Get(i, out name, out value);
					this.attrList.Put(name, value);
				}
				this.attrValueList.Clear();
			}
			string name2 = this.MemberElementName(nameInfo, typeNameInfo);
			this.NamespaceAttribute();
			this.Write(InternalElementTypeE.ObjectBegin, name2, this.attrList, null, true, false);
		}
		...
		internal void WriteAttributeValue(NameInfo memberNameInfo, NameInfo typeNameInfo, object value)
		{
			...
			this.attrValueList.Put(this.MemberElementName(memberNameInfo, typeNameInfo), value2);
		}
		...
		internal void WriteTopPrimitive(NameInfo nameInfo, object value)
		{
			...
			{
				value2 = Converter.SoapToString(value, nameInfo.NIprimitiveTypeEnum);
			}
			this.Write(InternalElementTypeE.Member, "xsd:" + Converter.ToXmlDataType(nameInfo.NIprimitiveTypeEnum), this.attrList, value2, true, false);
		}
		...
		internal void WriteObjectByteArray(NameInfo memberNameInfo, NameInfo arrayNameInfo, WriteObjectInfo objectInfo, NameInfo arrayElemTypeNameInfo, int length, int lowerBound, byte[] byteA)
		{
			...
			this.NamespaceAttribute();
			this.Write(InternalElementTypeE.Member, name, this.attrList, value, true, false);
		}
		...
		internal void WriteMember(NameInfo memberNameInfo, NameInfo typeNameInfo, object value)
		{
			...
			string name = this.MemberElementName(memberNameInfo, typeNameInfo2);
			this.NamespaceAttribute();
			this.Write(InternalElementTypeE.Member, name, this.attrList, value2, true, Converter.IsEscaped(typeNameInfo.NIprimitiveTypeEnum));
		}
		...
		internal void WriteItem(NameInfo itemNameInfo, NameInfo typeNameInfo, object value)
		{
			...
			this.NamespaceAttribute();
			this.Write(InternalElementTypeE.Member, "item", this.attrList, value2, false, typeNameInfo.NIprimitiveTypeEnum == InternalPrimitiveTypeE.Invalid);
		}
		...
		internal void WriteHeaderEntry(NameInfo nameInfo, NameInfo typeNameInfo, object value)
		{
			...
			this.NamespaceAttribute();
			this.Write(InternalElementTypeE.Member, nameInfo.NIheaderPrefix + ":" + nameInfo.NIname, this.attrList, value2, true, true);
		}
		...
		private SoapWriter.DottedInfo ParseAssemblyName(string typeFullName, string assemName)
		{
			...
			else
			{
				...
				string name = typeFullName.Substring(num + 1);
				int num2;
		...
		private string MemberElementName(NameInfo memberNameInfo, NameInfo typeNameInfo)
		{
			...
			{
				result = this.NameTagResolver(typeNameInfo, true);
			}
			return result;
		}
		...
		private string TypeNameTagResolver(NameInfo typeNameInfo, bool isXsiAppended)
		{
			...
			if (typeNameInfo.NIassemId > 0L && typeNameInfo.NIattributeInfo != null && typeNameInfo.NIattributeInfo.AttributeTypeName != null)
			{
				...
				result = text + ":" + typeNameInfo.NIattributeInfo.AttributeTypeName;
			...
			{
				result = this.NameTagResolver(typeNameInfo, isXsiAppended);
			}
			return result;
		}
		...
		private string NameTagResolver(NameInfo typeNameInfo, bool isXsiAppended, string arrayItemName)
		{
			...
			case InternalNameSpaceE.UrtUser:
				if (typeNameInfo.NIassemId > 0L)
				{
					if (arrayItemName == null)
					{
						if (!this.typeNameToDottedInfoTable.ContainsKey(typeNameInfo.NIname))
						{
							throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Assembly"), new object[]
							{
								typeNameInfo.NIname
							}));
						}
			...
			case InternalNameSpaceE.CallElement:
				if (typeNameInfo.NIassemId > 0L)
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_NameSpaceEnum"), new object[]
						{
							typeNameInfo.NInameSpaceEnum
						}));
					}
		...
		private string TypeArrayNameTagResolver(NameInfo memberNameInfo, NameInfo typeNameInfo, bool isXsiAppended)
		{
			...
			if (typeNameInfo.NIassemId > 0L && typeNameInfo.NIattributeInfo != null && typeNameInfo.NIattributeInfo.AttributeTypeName != null)
			{
				result = this.InteropAssemIdToString((int)typeNameInfo.NIassemId) + ":" + typeNameInfo.NIattributeInfo.AttributeTypeName;
			}
			else
			{
				result = this.NameTagResolver(typeNameInfo, isXsiAppended, memberNameInfo.NIname);
			}
			return result;
		}
