...
namespace System.Runtime.Serialization.Formatters.Soap
{
	...
	internal sealed class SoapHandler
	{
		...
		private void MarshalError(string handler, ParseRecord pr, string value, InternalParseStateE currentState)
		{
			...
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Syntax"), new object[]
			{
				text
			}));
		}
		...
		internal void StartElement(string prefix, string name, string urn)
		{
			...
			case InternalParseStateE.Object:
			{
				...
				else if (string.Compare(urn, "http://schemas.xmlsoap.org/soap/envelope/", StringComparison.OrdinalIgnoreCase) == 0)
				{
					if (string.Compare(name, "Envelope", StringComparison.OrdinalIgnoreCase) == 0)
					{
						if (this.isEnvelope)
						{
							throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_Envelope"), new object[]
							{
								prefix + ":" + name
							}));
						}
			...
			case InternalParseStateE.MemberChild:
			{
				...
				this.ProcessAttributes(parseRecord, objectPr);
		...
		{
			string value = this.NameFilter(name);
			switch (this.currentState)
			{
			case InternalParseStateE.Object:
			{
			...
			case InternalParseStateE.MemberChild:
			{
				...
				{
					this.MarshalError("EndElement", (ParseRecord)this.stack.Peek(), value, this.currentState);
					return;
				}
				break;
			}
		...
		private void ProcessAttributes(ParseRecord pr, ParseRecord objectPr)
		{
			...
			if (text3 != null)
			{
				...
				else
				{
					if (!text3.Equals("0"))
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MustUnderstand"), new object[]
						{
							text3
						}));
					}
		...
		private void ProcessType(ParseRecord pr, ParseRecord objectPr)
		{
			...
			else if (pr.PRname != null)
			{
				...
				if (objectPr != null)
				{
					...
					{
						SoapServices.GetInteropFieldTypeAndNameFromXmlAttribute(objectPr.PRdtType, pr.PRname, text, out type, out prname);
					}
					else
					{
						SoapServices.GetInteropFieldTypeAndNameFromXmlElement(objectPr.PRdtType, pr.PRname, text, out type, out prname);
					}
				}
				if (type != null)
				{
			...
			if (pr.PRtypeXmlKey != null && pr.PRtypeXmlKey.Length > 0 && pr.PRkeyDt != null && pr.PRkeyDt.Length > 0 && this.assemKeyToInteropAssemblyTable.ContainsKey(pr.PRtypeXmlKey))
			{
				...
				string text2 = (string)this.assemKeyToInteropAssemblyTable[pr.PRtypeXmlKey];
				pr.PRdtType = this.objectReader.Bind(text2, pr.PRkeyDt);
				if (pr.PRdtType == null)
				{
					pr.PRdtType = SoapServices.GetInteropTypeFromXmlType(pr.PRkeyDt, text2);
					if (pr.PRdtType == null)
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeElement"), new object[]
						{
							pr.PRname + " " + pr.PRkeyDt
						}));
					}
				}
				if (pr.PRdtType == null)
			...
			else if (pr.PRkeyDt != null)
			{
				...
				if (string.Compare(pr.PRkeyDt, "methodSignature", StringComparison.OrdinalIgnoreCase) == 0)
				{
					try
					{
						...
						else
						{
							...
							for (int i = 0; i < array.Length; i += 2)
							{
								...
								array2[i / 2] = this.ProcessGetType(value, xmlKey, out pr.PRassemblyName);
							}
						}
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MethodSignature"), new object[]
						{
							pr.PRvalue
						}));
					}
		...
		private Type ProcessGetType(string value, string xmlKey, out string assemblyString)
		{
			...
			else if (xmlKey.Equals(this.xsdKey) || xmlKey.Equals(this.soapKey))
			{
				...
				else
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xsd"), new object[]
						{
							value
						}));
					}
			...
			else
			{
				if (xmlKey == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xml"), new object[]
					{
						value
					}));
				}
				...
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xmlAssembly"), new object[]
					{
						xmlKey + " " + value
					}));
				}
			...
			if (type == null)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xmlType"), new object[]
				{
					string.Concat(new string[]
					{
						xmlKey,
		...
		private void ProcessArray(ParseRecord pr, int firstIndex, bool IsInterop)
		{
			...
			if (num2 < 1)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArrayDimensions"), new object[]
				{
					pr.PRkeyDt
				}));
			}
			int prrank = 0;
			string value = null;
			InternalArrayTypeE prarrayTypeEnum = InternalArrayTypeE.Empty;
			int num3 = 0;
			...
			{
				num3++;
				prlengthA = this.ParseArrayDimensions(pr.PRkeyDt.Substring(num, num2 - num + 1), out prrank, out value, out prarrayTypeEnum);
			...
			pr.PRlengthA = prlengthA;
			pr.PRrank = prrank;
			if (num3 == 1)
			{
				pr.PRarrayElementTypeCode = internalPrimitiveTypeE;
				pr.PRarrayTypeEnum = prarrayTypeEnum;
			...
			if (!IsInterop || num3 > 1)
			{
				...
				if (pr.PRarrayElementType == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArrayType"), new object[]
					{
		...
		private void ProcessHeaderMemberEnd(ParseRecord pr)
		{
			...
			ParseRecord pr2 = this.GetPr();
			pr2.PRparseTypeEnum = InternalParseTypeE.MemberEnd;
			pr2.PRmemberTypeEnum = InternalMemberTypeE.Field;
			pr2.PRmemberValueEnum = pr.PRmemberValueEnum;
			pr2.PRisHeaderRoot = true;
			this.headerList.Add(pr2);
