	...
	public class PrinterSettings : ICloneable
	{
			...
			if (image.RawFormat.Equals(ImageFormat.Jpeg) || image.RawFormat.Equals(ImageFormat.Png))
			{
				...
				try
				{
					...
					using (BufferedStream bufferedStream = new BufferedStream(memoryStream))
					{
						...
						try
						{
							bool flag = SafeNativeMethods.ExtEscape(hDC, 8, Marshal.SizeOf(typeof(int)), ref nEscape, 0, out num2) > 0;
							if (flag)
							{
								result = (SafeNativeMethods.ExtEscape(hDC, nEscape, num, array, Marshal.SizeOf(typeof(int)), out num2) > 0 && num2 == 1);
							}
						}
						finally
						{
		...
		internal PaperSource[] Get_PaperSources()
		{
			...
			for (int i = 0; i < num; i++)
			{
				checked
				{
					string name = Marshal.PtrToStringAuto((IntPtr)((long)intPtr + unchecked((long)(checked(num2 * i)))));
