	...
	public class InPlaceHostingManager : IDisposable
	{
		...
		public ObjectHandle Execute()
		{
			...
			lock (this._lock)
			{
				...
				this.ChangeState(InPlaceHostingManager.State.Done);
				result = this._deploymentManager.ExecuteNewDomain();
			}
			return result;
		}
		...
		private static DefinitionIdentity GetSubIdAndValidate(string subscriptionId)
		{
			...
			{
				throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Resources.GetString("Ex_ComArgSubIdentityNotValid"), new object[]
				{
					subscriptionId
				}), innerException);
			}
		...
		private void OnBindCompleted(object sender, BindCompletedEventArgs e)
		{
			lock (this._lock)
			{
				...
				GetManifestCompletedEventArgs e2 = null;
				try
				{
					...
					{
						e2 = new GetManifestCompletedEventArgs(e, this._deploymentManager.LogFilePath);
					}
					else
					{
						...
						{
							e2 = new GetManifestCompletedEventArgs(e, new InvalidOperationException(Resources.GetString("Ex_HostInBrowserFlagMustBeTrue")), this._deploymentManager.LogFilePath);
						}
						else if (install && (this._isLaunchInHostProcess || this._appType == AppType.CustomHostSpecified))
						{
							e2 = new GetManifestCompletedEventArgs(e, new InvalidOperationException(Resources.GetString("Ex_InstallFlagMustBeFalse")), this._deploymentManager.LogFilePath);
						}
						else if (useManifestForTrust && this._appType == AppType.CustomHostSpecified)
						{
							e2 = new GetManifestCompletedEventArgs(e, new InvalidOperationException(Resources.GetString("Ex_CannotHaveUseManifestForTrustFlag")), this._deploymentManager.LogFilePath);
						}
						else if (providerCodebaseUri != null && this._appType == AppType.CustomHostSpecified)
						{
							e2 = new GetManifestCompletedEventArgs(e, new InvalidOperationException(Resources.GetString("Ex_CannotHaveDeploymentProvider")), this._deploymentManager.LogFilePath);
						}
						else if (hostInBrowser && this._appType == AppType.CustomUX)
						{
							e2 = new GetManifestCompletedEventArgs(e, new InvalidOperationException(Resources.GetString("Ex_CannotHaveCustomUXFlag")), this._deploymentManager.LogFilePath);
						}
						else
						{
							e2 = new GetManifestCompletedEventArgs(e, this._deploymentManager.ActivationDescription, this._deploymentManager.LogFilePath);
						}
					}
				}
				catch
				...
				this.GetManifestCompleted(this, e2);
			}
		}
		...
		private void OnSynchronizeCompleted(object sender, SynchronizeCompletedEventArgs e)
		{
			lock (this._lock)
			{
				...
				{
					DownloadApplicationCompletedEventArgs e2 = new DownloadApplicationCompletedEventArgs(e, this._deploymentManager.LogFilePath, this._deploymentManager.ShortcutAppId);
