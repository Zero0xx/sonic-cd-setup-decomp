...
namespace System.Deployment.Application
{
	...
	internal class DeploymentManager : IDisposable, IDownloadNotification
	{
		...
		internal DeploymentManager(string appId, bool isUpdate, bool isConfirmed, DownloadOptions downloadOptions, AsyncOperation optionalAsyncOp) : this(null, isUpdate, isConfirmed, downloadOptions, optionalAsyncOp)
		{
		...
		public void SynchronizeAsync(string groupName, object userState)
		{
			...
			throw new InvalidOperationException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_SyncGroupOnce"), new object[]
			{
				groupName
			}));
		}
		...
		public string ShortcutAppId
		{
			get
			{
				...
				{
					result = string.Format("{0}#{1}", subscriptionState.DeploymentProviderUri.AbsoluteUri, subscriptionState.SubscriptionId.ToString());
				}
				return result;
			}
		}
		...
		public string LogFilePath
		{
			get
			{
				string result = Logger.GetLogFilePath(this._log);
		...
		private void BindAsyncCompleted(object arg)
		{
			...
			{
				bindCompletedEventHandler(this, e);
			}
		}
		...
		private void SynchronizeAsyncCompleted(object arg)
		{
			...
			{
				synchronizeCompletedEventHandler(this, e);
			}
		}
		...
		private void BindAsyncWorker()
		{
			...
			try
			{
				flag = this.BindCore(false, ref tempFile, ref tempDirectory, ref fileStream, ref name);
			}
			catch (Exception ex2)
			{
			...
			finally
			{
				...
				BindCompletedEventArgs arg = new BindCompletedEventArgs(ex, flag, null, this._actCtx, name, this._cached);
		...
		private bool BindCore(bool blocking, ref TempFile tempDeploy, ref TempDirectory tempAppDir, ref FileStream refTransaction, ref string productName)
		{
			try
			{
				...
				bool flag = this._subStore.CheckAndReferenceApplication(subscriptionState, definitionAppId, transactionId);
				...
				Uri uri;
				string manifestPath;
				AssemblyManifest assemblyManifest2 = DownloadManager.DownloadApplicationManifest(activationDescription.DeployManifest, tempAppDir.Path, activationDescription.DeploySourceUri, blocking ? null : this, this._downloadOptions, out uri, out manifestPath);
				...
				activationDescription.SetApplicationManifest(assemblyManifest2, uri, manifestPath);
				...
				flag = this._subStore.CheckAndReferenceApplication(subscriptionState, activationDescription.AppId, transactionId);
				if (!blocking && this._cancellationPending)
				{
					return true;
				}
		...
		private bool BindCoreWithAppId(bool blocking, ref FileStream refTransaction, ref string productName)
		{
			...
			long transactionId;
			refTransaction = this._subStore.AcquireReferenceTransaction(out transactionId);
			bool cached = this._subStore.CheckAndReferenceApplication(subscriptionState, this._bindAppId, transactionId);
			ActivationDescription activationDescription = new ActivationDescription();
		...
		private void CheckSizeLimit()
		{
			...
			if (!flag && flag2)
			{
				...
				{
					throw new DeploymentDownloadException(ExceptionTypes.SizeLimitForPartialTrustOnlineAppExceeded, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_OnlineSemiTrustAppSizeLimitExceeded"), new object[]
					{
						sizeLimitInBytesForSemiTrustApps
					}));
				}
			}
		}
		...
		private void SynchronizeGroupAsyncWorker(object arg)
		{
			...
			try
			{
				...
				cancelled = this.SynchronizeGroupCore(false, syncGroupHelper);
			}
			catch (Exception ex)
			{
				if (ex is DownloadCancelledException)
			...
			finally
			{
				...
				this.asyncOperation.Post(this.synchronizeCompleted, arg2);
			}
		}
		...
		private static ActivationContext ConstructActivationContext(ActivationDescription actDesc)
		{
			...
			return ActivationContext.CreatePartialActivationContext(identity, new string[]
			{
