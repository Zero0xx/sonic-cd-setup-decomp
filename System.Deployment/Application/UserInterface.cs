...
namespace System.Deployment.Application
{
	...
	internal class UserInterface : IDisposable
	{
		...
		{
			this.WaitReady();
			return (ProgressPiece)this._uiForm.Invoke(new UserInterfaceForm.ConstructProgressPieceDelegate(this._uiForm.ConstructProgressPiece), new object[]
			{
				info
			});
		}
		...
		public UserInterfaceModalResult ShowUpdate(UserInterfaceInfo info)
		{
			...
			UpdatePiece updatePiece = (UpdatePiece)this._uiForm.Invoke(new UserInterfaceForm.ConstructUpdatePieceDelegate(this._uiForm.ConstructUpdatePiece), new object[]
			{
				info,
				manualResetEvent
			});
		...
		public UserInterfaceModalResult ShowMaintenance(UserInterfaceInfo info, MaintenanceInfo maintenanceInfo)
		{
			this.WaitReady();
			ManualResetEvent manualResetEvent = new ManualResetEvent(false);
			MaintenancePiece maintenancePiece = (MaintenancePiece)this._uiForm.Invoke(new UserInterfaceForm.ConstructMaintenancePieceDelegate(this._uiForm.ConstructMaintenancePiece), new object[]
			{
				info,
				maintenanceInfo,
				manualResetEvent
		...
		{
			this.WaitReady();
			this._uiForm.Invoke(new UserInterfaceForm.ShowSimpleMessageBoxDelegate(this._uiForm.ShowSimpleMessageBox), new object[]
			{
				message,
				caption
			});
		...
		public void ShowPlatform(string platformDetectionErrorMsg, Uri supportUrl)
		{
			this.WaitReady();
			ManualResetEvent manualResetEvent = new ManualResetEvent(false);
			this._uiForm.BeginInvoke(new UserInterfaceForm.ConstructPlatformPieceDelegate(this._uiForm.ConstructPlatformPiece), new object[]
			{
				platformDetectionErrorMsg,
				supportUrl,
				manualResetEvent
			});
			manualResetEvent.WaitOne();
		...
		public void ShowError(string title, string message, string logFileLocation, string linkUrl, string linkUrlMessage)
		{
			this.WaitReady();
			ManualResetEvent manualResetEvent = new ManualResetEvent(false);
			this._uiForm.BeginInvoke(new UserInterfaceForm.ConstructErrorPieceDelegate(this._uiForm.ConstructErrorPiece), new object[]
			{
				title,
				message,
				logFileLocation,
