...
namespace System.Deployment.Application
{
	...
	internal abstract class FileDownloader
	{
		...
		private static FileStream GetPatchSourceStream(string filePath)
		{
			...
			{
				Logger.AddErrorInformation(exception, Resources.GetString("Ex_PatchSourceOpenFailed"), new object[]
				{
			...
			{
				Logger.AddErrorInformation(exception2, Resources.GetString("Ex_PatchSourceOpenFailed"), new object[]
				{
					Path.GetFileName(filePath)
				});
			}
		...
		{
			bool result = false;
			if (System.IO.File.Exists(location) && FileDownloader.FileHashVerified(hashCollection, location))
			{
		...
		private static void AddFilesInHashtable(Hashtable hashtable, AssemblyManifest applicationManifest, string applicationFolder)
		{
			...
			foreach (System.Deployment.Application.Manifest.File file in files)
			{
				...
				{
					Logger.AddErrorInformation(exception, Resources.GetString("Ex_PatchDependencyFailed"), new object[]
					{
			...
			foreach (DependentAssembly dependentAssembly in applicationManifest.DependentAssemblies)
			{
				if (!dependentAssembly.IsPreRequisite)
				{
					...
					try
					{
						if (FileDownloader.AddSingleFileInHashtable(hashtable, dependentAssembly.HashCollection, text))
						{
							...
							for (int k = 0; k < files2.Length; k++)
							{
								...
								FileDownloader.AddSingleFileInHashtable(hashtable, files2[k].HashCollection, location);
							}
						}
					}
					catch (InvalidDeploymentException exception2)
					{
						Logger.AddErrorInformation(exception2, Resources.GetString("Ex_PatchDependencyFailed"), new object[]
						{
							Path.GetFileName(text)
						});
					}
					...
					{
						Logger.AddErrorInformation(exception3, Resources.GetString("Ex_PatchDependencyFailed"), new object[]
						{
							Path.GetFileName(text)
						});
					}
