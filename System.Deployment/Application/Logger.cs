	...
	internal class Logger
	{
		...
		protected static Encoding LogFileEncoding
		{
			get
			{
				if (Logger._logFileEncoding == null)
				{
					...
					Interlocked.CompareExchange(ref Logger._logFileEncoding, value, null);
				}
		...
		protected static Logger GetCurrentThreadLogger()
		{
			...
			lock (Logger._logAccessLock)
			{
				if (Logger._threadLogIdTable.Contains(currentLogThreadId))
				{
					...
					{
						result = (Logger)Logger._loggerCollection[logIdentity.ToString()];
					}
				}
			}
			return result;
		...
		protected static Logger.HeaderSection Header
		{
			get
			{
				...
				{
					object value = new Logger.HeaderSection();
					Interlocked.CompareExchange(ref Logger._header, value, null);
				}
		...
		internal static void SetSubscriptionUrl(string subscrioptionUrl)
		{
			try
			{
				...
				Logger.SetSubscriptionUrl(subscriptionUrl);
			}
		...
		protected class ErrorInformation : Logger.LogInformation
		{
			...
			public string Summary
			{
				...
				{
					StringBuilder stringBuilder = new StringBuilder();
					stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_IndividualErrorSummary"), new object[]
					{
						this._message
					});
					for (Exception ex = this._exception; ex != null; ex = ex.InnerException)
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_IndividualErrorSummaryBullets"), new object[]
						{
							ex.Message
						});
					}
			...
			private static string GetExceptionType(Exception exception)
			{
				...
				return string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_ExceptionTypeUnknown"), new object[]
				{
					ex.GetType().ToString()
				});
			}
		...
		protected class TransactionInformation : Logger.LogInformation
		{
			...
			public TransactionInformation(StoreTransactionOperation[] storeOperations, uint[] rgDispositions, int[] rgResults, DateTime time) : base(null, time)
			{
			...
			public string FailureSummary
			{
				get
				{
					...
					{
						StringBuilder stringBuilder = new StringBuilder();
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionFailureSummaryItem"), new object[]
						{
							base.Time.ToString(DateTimeFormatInfo.CurrentInfo)
						});
						foreach (object obj in this._operations)
						{
							...
							if (transactionOperation.Failed)
							{
								stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionFailureSummaryBullets"), new object[]
								{
									transactionOperation.FailureMessage
								});
							}
						}
						return stringBuilder.ToString();
			...
			{
				StringBuilder stringBuilder = new StringBuilder();
				stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionItem"), new object[]
				{
					base.Time.ToString(DateTimeFormatInfo.CurrentInfo)
				});
				foreach (object obj in this._operations)
				{
					Logger.TransactionInformation.TransactionOperation transactionOperation = (Logger.TransactionInformation.TransactionOperation)obj;
					stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionBullets"), new object[]
					{
				...
				protected void AnalyzeTransactionOperation(StoreTransactionOperation operation, uint dispositionValue, int hresult)
				{
					try
					{
						if (operation.Operation == StoreTransactionOperationType.StageComponent)
						{
							...
							{
								this._failureMessage = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionStageComponentFailure"), new object[]
								{
									Path.GetFileName(storeOperationStageComponent.ManifestPath)
								});
								this._failed = true;
						...
						else if (operation.Operation == StoreTransactionOperationType.PinDeployment)
						{
							...
							if (dispositionValue == 0U)
							{
								this._failureMessage = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionPinDeploymentFailure"), new object[]
								{
									definitionAppId.ToString()
								});
								this._failed = true;
						...
						else if (operation.Operation == StoreTransactionOperationType.UnpinDeployment)
						{
							...
							if (dispositionValue == 0U)
							{
								this._failureMessage = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionUnPinDeploymentFailure"), new object[]
								{
									definitionAppId2.ToString()
								});
								this._failed = true;
						...
						else if (operation.Operation == StoreTransactionOperationType.InstallDeployment)
						{
							...
							if (dispositionValue == 0U)
							{
								this._failureMessage = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionInstallDeploymentFailure"), new object[]
								{
									definitionAppId3.ToString()
								});
								this._failed = true;
						...
						else if (operation.Operation == StoreTransactionOperationType.UninstallDeployment)
						{
							...
							if (dispositionValue == 0U)
							{
								this._failureMessage = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionUninstallDeploymentFailure"), new object[]
								{
									definitionAppId4.ToString()
								});
								this._failed = true;
						...
						else if (operation.Operation == StoreTransactionOperationType.StageComponentFile)
						{
							...
							if (dispositionValue == 0U)
							{
								this._failureMessage = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_TransactionStageComponentFileFailure"), new object[]
								{
									storeOperationStageComponentFile.ComponentRelativePath
								});
								this._failed = true;
		...
		protected class HeaderSection : Logger.LogInformation
		{
			...
			protected static string GetModulePathInSystemFolder(string moduleName)
			{
				...
				try
				{
					result = Path.Combine(Environment.SystemDirectory, moduleName);
				}
				catch (ArgumentException)
				{
					result = null;
			...
			protected static string GetModulePathInClrFolder(string moduleName)
			{
				...
				try
				{
					if (loadedModulePath != null)
					{
						...
						result = Path.Combine(directoryName, moduleName);
					}
				}
				catch (ArgumentException)
				{
			...
			{
				string modulePath = Logger.HeaderSection.GetExecutingAssemblyPath();
				string modulePath2 = Logger.HeaderSection.GetModulePath("mscorwks.dll");
				string modulePath3 = Logger.HeaderSection.GetModulePath("dfdll.dll");
				string modulePath4 = Logger.HeaderSection.GetModulePath("dfshim.dll");
				FileVersionInfo versionInfo = Logger.HeaderSection.GetVersionInfo(modulePath);
				if (versionInfo == null)
				{
					modulePath = Logger.HeaderSection.GetModulePathInClrFolder("system.deployment.dll");
					versionInfo = Logger.HeaderSection.GetVersionInfo(modulePath);
				}
				...
				FileVersionInfo versionInfo4 = Logger.HeaderSection.GetVersionInfo(modulePath4);
				StringBuilder stringBuilder = new StringBuilder();
				try
				{
					...
					stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_HeaderCLRVersion"), new object[]
					{
						Environment.Version.ToString()
					});
					if (versionInfo != null)
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_HeaderSystemDeploymentVersion"), new object[]
						{
					...
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_HeaderMscorwksVersion"), new object[]
						{
							versionInfo2.FileVersion
						});
					}
					if (versionInfo3 != null)
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_HeaderDfdllVersion"), new object[]
						{
							versionInfo3.FileVersion
						});
					}
					if (versionInfo4 != null)
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_HeaderDfshimVersion"), new object[]
						{
							versionInfo4.FileVersion
						});
					}
				}
				catch (ArgumentException)
		...
		protected class SourceSection : Logger.LogInformation
		{
			...
			public override string ToString()
			{
				...
				if (this._subscriptonUri != null || this._deploymentProviderUri != null || this._applicationUri != null)
				{
					...
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_SourceDeploymentUrl"), new object[]
						{
							this._subscriptonUri.AbsoluteUri
						});
					}
					...
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_SourceDeploymentProviderUrl"), new object[]
						{
							this._deploymentProviderUri.AbsoluteUri
						});
					}
					...
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_SourceApplicationUrl"), new object[]
						{
							this._applicationUri.AbsoluteUri
						});
					}
			...
			private static void AppendServerInformation(StringBuilder destination, ServerInformation serverInformation)
			{
				...
				{
					destination.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_ServerInformationServer"), new object[]
					{
				...
				{
					destination.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_ServerInformationPoweredBy"), new object[]
					{
						serverInformation.PoweredBy
					});
				}
				if (!string.IsNullOrEmpty(serverInformation.AspNetVersion))
				{
					destination.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_ServerInformationAspNetVersion"), new object[]
					{
						serverInformation.AspNetVersion
					});
				}
			}
		...
		protected class IdentitySection : Logger.LogInformation
		{
			...
			public override string ToString()
			{
				...
				if (this._deploymentIdentity != null || this._applicationIdentity != null)
				{
					...
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_IdentityDeploymentIdentity"), new object[]
						{
							this._deploymentIdentity.ToString()
						});
					}
					if (this._applicationIdentity != null)
					{
						stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_IdentityApplicationIdentity"), new object[]
						{
							this._applicationIdentity.ToString()
						});
					}
					stringBuilder.Append(Environment.NewLine);
				}
		...
		protected class ErrorSection : Logger.LogInformation
		{
			...
			{
				Logger.ErrorInformation value = new Logger.ErrorInformation(message, exception, time);
		...
		protected class WarningSection : Logger.LogInformation
		{
			...
			{
				Logger.LogInformation value = new Logger.LogInformation(message, time);
				this._warnings.Add(value);
			}
				...
				if (this._warnings.Count > 0)
				{
					using (IEnumerator enumerator = this._warnings.GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							...
							stringBuilder.AppendFormat(CultureInfo.CurrentUICulture, Resources.GetString("LogFile_WarningStatusIndivualWarning"), new object[]
							{
		...
		protected class PhaseSection : Logger.LogInformation
		{
			...
			{
				Logger.LogInformation value = new Logger.LogInformation(phaseMessage, time);
				this._phaseInformations.Add(value);
			}
		...
		public class LogIdentity
		{
			...
			public override string ToString()
			{
				...
				{
					this._logIdentityStringForm = string.Format(CultureInfo.InvariantCulture, "{0:x8}{1:x16}", new object[]
					{
						this._threadId,
						this._ticks
					});
