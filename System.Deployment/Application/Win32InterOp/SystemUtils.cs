...
namespace System.Deployment.Application.Win32InterOp
{
	...
	internal class SystemUtils
	{
		...
		public static byte[] GetManifestFromPEResources(string filePath)
		{
			...
			try
			{
				intPtr2 = NativeMethods.LoadLibraryEx(filePath, hFile, dwFlags);
				int lastWin32Error = Marshal.GetLastWin32Error();
				if (intPtr2 == IntPtr.Zero)
				{
					SystemUtils.Win32LoadExceptionHelper(lastWin32Error, "Ex_Win32LoadException", filePath);
				}
				intPtr = NativeMethods.FindResource(intPtr2, "#1", "#24");
				if (intPtr != IntPtr.Zero)
				{
					uint num = NativeMethods.SizeofResource(intPtr2, intPtr);
					lastWin32Error = Marshal.GetLastWin32Error();
					if (num == 0U)
					{
						SystemUtils.Win32LoadExceptionHelper(lastWin32Error, "Ex_Win32ResourceLoadException", filePath);
					}
					...
					array = new byte[num];
					Marshal.Copy(intPtr4, array, 0, (int)num);
				}
			}
			finally
			{
		...
		private static void Win32LoadExceptionHelper(int win32ErrorCode, string resourceId, string filePath)
		{
			...
			string message = string.Format(CultureInfo.CurrentUICulture, Resources.GetString(resourceId), new object[]
			{
				fileName,
		...
		internal static DefinitionIdentity GetDefinitionIdentityFromManagedAssembly(string filePath)
		{
			...
			IReferenceIdentity idComPtr = (IReferenceIdentity)NativeMethods.GetAssemblyIdentityFromFile(filePath, ref guidOfType);
		...
		internal static void CheckSupportedImageAndCLRVersions(string path)
		{
			...
			uint num2;
			NativeMethods.GetRequestedRuntimeInfo(path, null, null, 0U, runtimeInfoFlags, null, 0U, out num2, stringBuilder, (uint)stringBuilder.Capacity, out num);
			if (stringBuilder[0] != 'v')
			{
			...
			uint num4 = uint.Parse((num3 >= 0) ? text.Substring(0, num3) : text, CultureInfo.InvariantCulture);
			if (num4 < 2U)
			{
