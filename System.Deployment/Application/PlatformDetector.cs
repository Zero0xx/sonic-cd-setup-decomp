...
namespace System.Deployment.Application
{
	...
	internal static class PlatformDetector
	{
		...
		{
			bool result = true;
			PlatformDetector.NameMap[] nmArray = new PlatformDetector.NameMap[]
			{
				new PlatformDetector.NameMap("x86", 8U),
				new PlatformDetector.Product("ia64", 2U),
				new PlatformDetector.Product("amd64", 4U)
			};
			uint num = PlatformDetector.NameMap.MapNameToMask(procArch, nmArray);
			num |= 65U;
		...
		public static bool VerifyOSDependency(ref PlatformDetector.OSDependency osd)
		{
			...
			ulong num = 0UL;
			uint dwTypeMask = 2U | ((osd.dwMinorVersion != 0U) ? 1U : 0U) | ((osd.dwBuildNumber != 0U) ? 4U : 0U) | ((osd.suiteName != null) ? 64U : 0U) | ((osd.productName != null) ? 128U : 0U) | ((osd.wServicePackMajor != 0) ? 32U : 0U) | ((osd.wServicePackMinor != 0) ? 16U : 0U);
			...
			bool flag = NativeMethods.VerifyVersionInfo(osversionInfoEx, dwTypeMask, num);
			if (!flag)
			{
		...
		public static bool VerifyGACDependencyWhidbey(ReferenceIdentity refId)
		{
			...
			SystemUtils.AssemblyInfo assemblyInfo = SystemUtils.QueryAssemblyInfo(SystemUtils.QueryAssemblyInfoFlags.All, assemblyName2);
			if (assemblyInfo == null && referenceIdentity.ProcessorArchitecture == null)
			{
				...
				NativeMethods.CreateAssemblyEnum(out assemblyEnum, null, pName, 2U, IntPtr.Zero);
				return assemblyEnum.GetNextAssembly(null, out pName, 0U) == 0;
			}
		...
		{
			Uri supportUrl = deploySupportUri;
			...
			if (dependentOS != null)
			{
				...
				if (!PlatformDetector.VerifyOSDependency(ref osdependency))
				{
					...
					{
						dependentOS.MajorVersion,
						".",
						dependentOS.MinorVersion,
						".",
						dependentOS.BuildNumber,
						".",
						dependentOS.ServicePackMajor,
						dependentOS.ServicePackMinor
					});
					...
					throw new DependentPlatformMissingException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ErrorMessage_PlatformDetectionFailed"), new object[]
					{
						text
					}), supportUrl);
				}
			...
			foreach (DependentAssembly dependentAssembly in appManifest.DependentAssemblies)
			{
				if (dependentAssembly.IsPreRequisite && PlatformDetector.IsCLRDependencyText(dependentAssembly.Identity.Name))
				{
					...
					if (!PlatformDetector.VerifyCLRVersionInfo(version, processorArchitecture))
					{
						...
						{
							supportUrl = dependentAssembly.SupportUrl;
						}
						throw new DependentPlatformMissingException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ErrorMessage_PlatformDetectionFailed"), new object[]
						{
							text
			...
			if (!PolicyKeys.SkipSKUDetection())
			{
				...
				{
					string text = ".NET Framework 3.5 SP1";
					throw new DependentPlatformMissingException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ErrorMessage_PlatformDetectionFailed"), new object[]
					{
						text
					}));
			...
			foreach (DependentAssembly dependentAssembly2 in appManifest.DependentAssemblies)
			{
				if (dependentAssembly2.IsPreRequisite && !PlatformDetector.IsCLRDependencyText(dependentAssembly2.Identity.Name) && !PlatformDetector.VerifyGACDependency(dependentAssembly2.Identity, tempDir))
				{
					...
					{
						supportUrl = dependentAssembly2.SupportUrl;
					}
					throw new DependentPlatformMissingException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ErrorMessage_PlatformGACDetectionFailed"), new object[]
					{
						text
					}), supportUrl);
				}
		...
		private static PlatformDetector.NetFX35SP1SKU GetPlatformNetFx35SKU(string tempDir)
		{
			...
			bool flag = false;
			bool flag2 = false;
			if (PlatformDetector.VerifyGACDependency(refId, tempDir))
			{
				flag = true;
			}
			if (PlatformDetector.VerifyGACDependency(refId2, tempDir))
			{
				flag2 = true;
			}
			if (flag && !flag2)
