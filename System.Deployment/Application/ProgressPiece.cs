	...
	internal class ProgressPiece : FormPiece, IDownloadNotification
	{
		...
		private void UpdateUI()
		{
			...
			if (bytesTotal > 2147483647L)
			{
				...
				value = (int)((float)this._downloadData.BytesCompleted / num);
				maximum = int.MaxValue;
			}
			else
			{
				value = (int)this._downloadData.BytesCompleted;
				maximum = (int)bytesTotal;
			}
		...
		private static string FormatBytes(long bytes)
		{
			...
			num = ((num >= 0) ? (num + 1) : (~num));
			return string.Format(CultureInfo.CurrentUICulture, Resources.GetString(ProgressPiece._bytesFormatStrings[num]), new object[]
			{
				(num == 0) ? ((float)bytes) : ((float)bytes / (float)ProgressPiece._bytesFormatRanges[(num - 1) / 3 * 3])
			});
		}
		...
		private static long[] _bytesFormatRanges = new long[]
		{
			1024L,
			10240L,
			102400L,
		...
		private static string[] _bytesFormatStrings = new string[]
		{
