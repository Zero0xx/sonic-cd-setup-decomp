...
namespace System.Deployment.Application
{
	...
	internal class PEStream : Stream
	{
		...
		private void ConstructFromFile(string filePath, bool partialConstruct)
		{
			...
			{
				throw new IOException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_InvalidPEImage"), new object[]
				{
					fileName
				}), innerException);
			}
		...
		protected void ConstructPEImage(FileStream file, bool partialConstruct)
		{
			...
			foreach (object obj in this._streamComponents)
			{
				...
				if (pecomponent.Address > num5)
				{
					PEStream.PEComponent value = new PEStream.PEComponent(file, num5, pecomponent.Address - num5);
					arrayList.Add(value);
				}
			...
			if (num5 < file.Length)
			{
				PEStream.PEComponent value2 = new PEStream.PEComponent(file, num5, file.Length - num5);
		...
		protected void ConstructStream()
		{
			...
			{
				PEStream.DataDirectory peComponent = (PEStream.DataDirectory)obj;
				this._streamComponents.Add(peComponent);
			}
			foreach (object obj2 in this._sectionHeaders)
			{
				PEStream.SectionHeader peComponent2 = (PEStream.SectionHeader)obj2;
				this._streamComponents.Add(peComponent2);
			}
		...
		protected class PEComponent
		{
			...
			public virtual int Read(byte[] buffer, int bufferOffset, long sourceOffset, int count)
			{
				...
				if (this._data is PEStream.DataComponent)
				{
					...
					result = dataComponent.Read(buffer, bufferOffset, sourceOffset, (int)num);
				}
				else
				{
					byte[] array = PEStream.PEComponent.ToByteArray(this._data);
					long num2 = Math.Min((long)count, (long)array.Length - sourceOffset);
					if (num2 < 0L)
					{
						throw new ArgumentException(Resources.GetString("Ex_InvalidCopyRequest"));
					}
					Array.Copy(array, (int)sourceOffset, buffer, bufferOffset, (int)num2);
					result = (int)num2;
				}
				return result;
			}
			...
			public ResourceDirectory(PEStream.ResourceSection resourceSection, FileStream file, long rootResourceAddress, long resourceAddress, long addressDelta, bool partialConstruct)
			{
				...
				foreach (object obj in this._resourceDirectoryEntries)
				{
					...
					if (resourceDirectoryEntry3.NameIsString)
					{
						...
						key = resourceDirectoryString.NameString;
					}
					else
					{
						key = resourceDirectoryEntry3.Id;
						if (rootResourceAddress == resourceAddress && resourceDirectoryEntry3.Id == 24)
						{
							flag = true;
						}
					}
					resourceDirectoryEntry3.Key = key;
					object obj2 = null;
					if (resourceDirectoryEntry3.IsDirectory)
					{
						if (!partialConstruct || (partialConstruct && flag))
						{
							obj2 = new PEStream.ResourceDirectory(resourceSection, file, rootResourceAddress, rootResourceAddress + resourceDirectoryEntry3.OffsetToData, addressDelta, false);
						}
					}
					else
					{
						obj2 = new PEStream.ResourceData(file, rootResourceAddress, rootResourceAddress + resourceDirectoryEntry3.OffsetToData, addressDelta);
					}
					if (obj2 != null)
					{
						this._resourceDirectoryItems.Add(key, obj2);
					}
				}
			}
