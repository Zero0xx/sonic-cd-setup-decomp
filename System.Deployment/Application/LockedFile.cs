...
namespace System.Deployment.Application
{
	...
	internal static class LockedFile
	{
		...
		public static IDisposable AcquireLock(string path, TimeSpan timeout, bool writer)
		{
			...
			DateTime t = DateTime.UtcNow + timeout;
			FileAccess access;
			...
			else
			{
				...
				dwDesiredAccess = (NativeMethods.GenericAccess)2147483648U;
			...
			for (;;)
			{
				safeFileHandle = NativeMethods.CreateFile(path, (uint)dwDesiredAccess, (uint)dwShareMode, IntPtr.Zero, 4U, 67108864U, IntPtr.Zero);
			...
			return new LockedFile.LockedFileHandle(safeFileHandle, path, access);
			Block_7:
