...
namespace System.Deployment.Application
{
	...
	internal class ComponentStore
	{
					...
					if (registryKey2 == null && registryKey3 != null)
					{
						...
						})
						{
							...
							foreach (DirectoryInfo directoryInfo3 in directoryInfo2.GetDirectories("Apps"))
							{
								...
								{
									foreach (DirectoryInfo directoryInfo5 in directoryInfo4.GetDirectories(searchPattern))
									{
										foreach (DirectoryInfo directoryInfo6 in directoryInfo5.GetDirectories(searchPattern2))
										{
		...
		internal ulong GetPrivateSize(ArrayList deployAppIds)
		{
			ulong result;
			ulong num;
			this.GetPrivateAndSharedSizes(deployAppIds, out result, out num);
			return result;
		}
		...
		internal ulong GetSharedSize(ArrayList deployAppIds)
		{
			ulong num;
			ulong result;
			this.GetPrivateAndSharedSizes(deployAppIds, out num, out result);
			return result;
		}
		...
		private int CalculateDeploymentsUnderQuota(int numberOfDeployments, IDefinitionAppId[] deployAppIdPtrs, ulong quotaSize, ref ulong privateSize, ref ulong sharedSize)
		{
			...
			this._store.CalculateDelimiterOfDeploymentsBasedOnQuota(0U, (uint)numberOfDeployments, deployAppIdPtrs, ref installReference, quotaSize, ref result, ref sharedSize, ref privateSize);
			return (int)result;
		}
		...
		public SubscriptionStateInternal GetSubscriptionStateInternal(DefinitionIdentity subId)
		{
			...
			{
				DefinitionAppId appId = new DefinitionAppId(new DefinitionIdentity[]
				{
					subId
				});
		...
		public void ActivateApplication(DefinitionAppId appId, string activationParameter, bool useActivationParameter)
		{
			...
			if (activationParameter != null && (assemblyManifest.Deployment.TrustURLParameters || useActivationParameter))
			{
				...
				activationData = new string[]
				{
					activationParameter
				};
			}
			uint num = (uint)hostType;
		...
		{
			Store.IPathLock result;
			try
			{
				result = this._store.LockApplicationPath(definitionAppId.ComPointer);
			}
			catch (COMException ex)
			{
				if (ex.ErrorCode == -2147024784)
		...
		public Store.IPathLock LockAssemblyPath(DefinitionIdentity asmId)
		{
			...
			try
			{
				result = this._store.LockAssemblyPath(asmId.ComPointer);
			}
			catch (COMException ex)
			{
				if (ex.ErrorCode == -2147024784)
		...
		public bool CheckGroupInstalled(DefinitionAppId appId, AssemblyManifest appManifest, string groupName)
		{
			...
			try
			{
				...
				{
					throw new InvalidDeploymentException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_NoSuchDownloadGroup"), new object[]
					{
						groupName
					}));
				}
		...
		{
			ComponentStore.HostType result = ComponentStore.HostType.Default;
			try
			{
				bool propertyBoolean = this.GetPropertyBoolean(defAppId, "IsFullTrust");
				if (propertyBoolean)
				{
					result = ComponentStore.HostType.CorFlag;
				}
				else
				{
					result = ComponentStore.HostType.AppLaunch;
				}
			}
		...
		private bool IsSubscriptionInstalled(DefinitionIdentity subId)
		{
			...
			try
			{
				DefinitionAppId propertyDefinitionAppId = this.GetPropertyDefinitionAppId(appId, "CurrentBind");
				result = (propertyDefinitionAppId != null);
			}
			catch (DeploymentException)
			{
				result = false;
		...
		private string GetPropertyString(DefinitionAppId appId, string propName)
		{
			...
			{
				throw new DeploymentException(ExceptionTypes.SubscriptionState, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_InvalidStoreMetaData"), new object[]
				{
					propName
				}));
			}
		...
		private DefinitionIdentity GetPropertyDefinitionIdentity(DefinitionAppId appId, string propName)
		{
			...
			try
			{
				...
				result = ((propertyString != null && propertyString.Length > 0) ? new DefinitionIdentity(propertyString) : null);
			}
		...
		{
			DefinitionAppId result;
			try
			{
				string propertyString = this.GetPropertyString(appId, propName);
				result = ((propertyString != null && propertyString.Length > 0) ? new DefinitionAppId(propertyString) : null);
			}
			catch (COMException innerException)
			{
				throw new DeploymentException(ExceptionTypes.SubscriptionState, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_InvalidStoreMetaData"), new object[]
		...
		private bool GetPropertyBoolean(DefinitionAppId appId, string propName)
		{
			...
			{
				string propertyString = this.GetPropertyString(appId, propName);
				result = (propertyString != null && propertyString.Length > 0 && Convert.ToBoolean(propertyString, CultureInfo.InvariantCulture));
			}
		...
		private Uri GetPropertyUri(DefinitionAppId appId, string propName)
		{
			...
			{
				string propertyString = this.GetPropertyString(appId, propName);
				result = ((propertyString != null && propertyString.Length > 0) ? new Uri(propertyString) : null);
			}
		...
		private Version GetPropertyVersion(DefinitionAppId appId, string propName)
		{
			...
			{
				string propertyString = this.GetPropertyString(appId, propName);
				result = ((propertyString != null && propertyString.Length > 0) ? new Version(propertyString) : null);
			}
		...
		private DateTime GetPropertyDateTime(DefinitionAppId appId, string propName)
		{
			...
			{
				string propertyString = this.GetPropertyString(appId, propName);
				result = ((propertyString != null && propertyString.Length > 0) ? DateTime.ParseExact(propertyString, "yyyy/MM/dd HH:mm:ss", DateTimeFormatInfo.InvariantInfo) : DateTime.MinValue);
			}
			catch (FormatException innerException)
			{
				throw new DeploymentException(ExceptionTypes.SubscriptionState, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_InvalidStoreMetaData"), new object[]
			...
			try
			{
				...
				else
				{
					...
					case 0:
						...
						break;
					case 1:
						result = AppType.Installed;
						break;
					case 2:
						result = AppType.Online;
						break;
					case 3:
						result = AppType.CustomHostSpecified;
						break;
					case 4:
						result = AppType.CustomUX;
						break;
					default:
						result = AppType.None;
						break;
		...
		private void PrepareSetDeploymentProperties(ComponentStore.StoreTransactionContext storeTxn, DefinitionAppId appId, CommitApplicationParams commitParams)
		{
			...
			{
				value = ComponentStore.ToPropertyString(commitParams.DeploySourceUri);
				value2 = ComponentStore.ToPropertyString(commitParams.AppSourceUri);
				...
				{
					value3 = ComponentStore.ToPropertyString(commitParams.Trust.DefaultGrantSet.PermissionSet.IsUnrestricted());
				}
			}
			StoreOperationMetadataProperty[] setProperties = new StoreOperationMetadataProperty[]
			{
				new StoreOperationMetadataProperty(Constants.DeploymentPropertySet, "DeploymentSourceUri", value),
				new StoreOperationMetadataProperty(Constants.DeploymentPropertySet, "ApplicationSourceUri", value2),
				new StoreOperationMetadataProperty(Constants.DeploymentPropertySet, "IsFullTrust", value3)
			};
		...
		private void PrepareStageAppComponent(ComponentStore.StoreTransactionContext storeTxn, CommitApplicationParams commitParams)
		{
			...
			System.Deployment.Application.Manifest.File[] filesInGroup = appManifest.GetFilesInGroup(appGroup, true);
			foreach (System.Deployment.Application.Manifest.File file in filesInGroup)
			{
			...
			foreach (DependentAssembly privAsm in privateAssembliesInGroup)
			{
		...
		private void PrepareInstallFile(ComponentStore.StoreTransactionContext storeTxn, System.Deployment.Application.Manifest.File file, DefinitionAppId appId, DefinitionIdentity asmId, string asmPayloadPath)
		{
			...
			storeTxn.Add(new StoreOperationStageComponentFile(appId.ComPointer, (asmId != null) ? asmId.ComPointer : null, name, srcFile));
		}
		...
		private void PrepareInstallPrivateAssembly(ComponentStore.StoreTransactionContext storeTxn, DependentAssembly privAsm, DefinitionAppId appId, string appPayloadPath)
		{
			...
			foreach (System.Deployment.Application.Manifest.File file in assemblyManifest.Files)
			{
		...
		private static void FinalizeSubscriptionState(SubscriptionStateInternal newState)
		{
			...
			DefinitionIdentity obj = (definitionAppId != null) ? definitionAppId.DeploymentIdentity : null;
		...
		private void PrepareSetSubscriptionProperties(ComponentStore.StoreTransactionContext storeTxn, SubscriptionState subState, SubscriptionStateInternal newState)
		{
			...
			{
				StoreOperationMetadataProperty[] setProperties = (StoreOperationMetadataProperty[])arrayList.ToArray(typeof(StoreOperationMetadataProperty));
				DefinitionAppId definitionAppId = new DefinitionAppId(new DefinitionIdentity[]
				{
					subState.SubscriptionId
				});
				storeTxn.Add(new StoreOperationSetDeploymentMetadata(definitionAppId.ComPointer, this.InstallReference, setProperties));
			}
		}
		...
		private static string ToPropertyString(object propValue)
		{
			...
			{
				result = ((bool)propValue).ToString(CultureInfo.InvariantCulture);
			}
			else if (propValue is DateTime)
			{
				result = ((DateTime)propValue).ToString("yyyy/MM/dd HH:mm:ss", DateTimeFormatInfo.InvariantInfo);
			}
			else if (propValue is Uri)
			{
				result = ((Uri)propValue).AbsoluteUri;
			}
			else
			{
		...
		private class ScavengeContext
		{
			...
			public void CheckQuotaAndScavenge()
			{
				...
				{
					IDefinitionAppId[] array;
					ComponentStore.ScavengeContext.SubInstance[] subs = this.CollectOnlineAppsMRU(out array);
					ulong num3 = 0UL;
					ulong num4 = 0UL;
					if (array.Length > 0)
					{
						this._compStore.CalculateDeploymentsUnderQuota(array.Length, array, ulong.MaxValue, ref num3, ref num4);
						if (num3 > onlineAppQuotaInBytes)
						{
							ulong quotaSize = onlineAppQuotaInBytes / 2UL;
							int num5 = this._compStore.CalculateDeploymentsUnderQuota(array.Length, array, quotaSize, ref num3, ref num4);
							bool flag;
							this.ScavengeAppsOverQuota(subs, array.Length - num5, out flag);
							if (flag)
							{
								this.CollectOnlineApps(out array);
								this._compStore.CalculateDeploymentsUnderQuota(array.Length, array, ulong.MaxValue, ref num3, ref num4);
							}
						}
					}
			...
			{
				DefinitionAppId deployAppId = appId.ToDeploymentAppId();
				ComponentStore.ScavengeContext.AddDeploymentToList(ref this._onlineDeploysToPin, deployAppId);
				if (appId.Equals(subState.CurrentBind) || appId.Equals(subState.PreviousBind))
				{
					ComponentStore.ScavengeContext.AddDeploymentToList(ref this._onlineDeploysToPinAlreadyPinned, deployAppId);
				}
			}
			...
			public void CleanOnlineAppCache()
			{
				...
				if (array2.Length > 0)
				{
					this._compStore.CalculateDeploymentsUnderQuota(array2.Length, array2, ulong.MaxValue, ref usage, ref num);
				}
			...
			{
				appExcluded = false;
				DateTime t = DateTime.UtcNow - Constants.OnlineAppScavengingGracePeriod;
				using (ComponentStore.StoreTransactionContext storeTransactionContext = new ComponentStore.StoreTransactionContext(this._compStore))
				{
