...
namespace System.Deployment.Application
{
	...
	internal static class DownloadManager
	{
		...
		public static AssemblyManifest DownloadDeploymentManifest(SubscriptionStore subStore, ref Uri sourceUri, out TempFile tempFile, IDownloadNotification notification, DownloadOptions options)
		{
			...
			try
			{
				ServerInformation subscriptionServerInformation;
				result = DownloadManager.DownloadDeploymentManifestDirect(subStore, ref sourceUri, out tempFile2, notification, options, out subscriptionServerInformation);
				Logger.SetSubscriptionServerInformation(subscriptionServerInformation);
				tempFile = (DownloadManager.FollowDeploymentProviderUri(subStore, ref result, ref sourceUri, out tempFile3, notification, options) ? tempFile3 : tempFile2);
			}
			finally
			{
		...
		public static bool FollowDeploymentProviderUri(SubscriptionStore subStore, ref AssemblyManifest deployment, ref Uri sourceUri, out TempFile tempFile, IDownloadNotification notification, DownloadOptions options)
		{
			...
			if (flag)
			{
				...
				if (!PolicyKeys.SkipDeploymentProvider() && providerCodebaseUri != null && !providerCodebaseUri.Equals(sourceUri))
				{
					...
					ServerInformation deploymentProviderServerInformation;
					try
					{
						assemblyManifest = DownloadManager.DownloadDeploymentManifestDirect(subStore, ref providerCodebaseUri, out tempFile, notification, options, out deploymentProviderServerInformation);
					}
					...
					Logger.SetDeploymentProviderServerInformation(deploymentProviderServerInformation);
		...
		public static AssemblyManifest DownloadDeploymentManifestBypass(SubscriptionStore subStore, ref Uri sourceUri, out TempFile tempFile, out SubscriptionState subState, IDownloadNotification notification, DownloadOptions options)
		{
			...
			{
				ServerInformation subscriptionServerInformation;
				result = DownloadManager.DownloadDeploymentManifestDirectBypass(subStore, ref sourceUri, out tempFile2, out subState, notification, options, out subscriptionServerInformation);
				Logger.SetSubscriptionServerInformation(subscriptionServerInformation);
				if (subState != null)
				{
					tempFile = tempFile2;
		...
		public static AssemblyManifest DownloadApplicationManifest(AssemblyManifest deploymentManifest, string targetDir, Uri deploymentUri, IDownloadNotification notification, DownloadOptions options, out Uri appSourceUri, out string appManifestPath)
		{
			...
			ServerInformation applicationServerInformation;
			AssemblyManifest assemblyManifest = DownloadManager.DownloadManifest(ref appSourceUri, appManifestPath, notification, options, AssemblyManifest.ManifestType.Application, out applicationServerInformation);
			Logger.SetApplicationUrl(appSourceUri);
			Logger.SetApplicationServerInformation(applicationServerInformation);
			...
			{
				throw new InvalidDeploymentException(ExceptionTypes.ManifestSemanticValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_DepSameDeploymentAndApplicationIdentity"), new object[]
				{
					assemblyManifest.Identity.ToString()
				}));
			}
		...
		private static AssemblyManifest DownloadDeploymentManifestDirectBypass(SubscriptionStore subStore, ref Uri sourceUri, out TempFile tempFile, out SubscriptionState subState, IDownloadNotification notification, DownloadOptions options, out ServerInformation serverInformation)
		{
			...
			try
			{
				...
				definitionAppId = new DefinitionAppId(uri.AbsoluteUri, new DefinitionIdentity[]
				{
					identity,
					definitionIdentity
				});
		...
		private static void AddDependencies(FileDownloader downloader, AssemblyManifest deployManifest, AssemblyManifest appManifest, Uri sourceUriBase, string targetDirectory, string group)
		{
			...
			foreach (System.Deployment.Application.Manifest.File file in filesInGroup)
			{
				...
				DownloadManager.AddFileToDownloader(downloader, deployManifest, appManifest, file, fileSourceUri, targetDirectory, file.NameFS, file.HashCollection);
				num += (long)file.Size;
			}
			...
			foreach (DependentAssembly dependentAssembly in privateAssembliesInGroup)
			{
				...
				DownloadManager.AddFileToDownloader(downloader, deployManifest, appManifest, dependentAssembly, fileSourceUri, targetDirectory, dependentAssembly.CodebaseFS, dependentAssembly.HashCollection);
		...
		private static void AddFileToDownloader(FileDownloader downloader, AssemblyManifest deployManifest, AssemblyManifest appManifest, object manifestElement, Uri fileSourceUri, string targetDirectory, string targetFileName, HashCollection hashCollection)
		{
			...
			DownloadManager.DependencyDownloadCookie cookie = new DownloadManager.DependencyDownloadCookie(manifestElement, deployManifest, appManifest);
		...
		private static void ProcessDownloadedFile(object sender, DownloadEventArgs e)
		{
			...
			{
				throw new InvalidDeploymentException(ExceptionTypes.AppFileLocationValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_DownloadAppFileAsmRedirected"), new object[]
				{
					fileName
				}));
			}
			...
			{
				if (dependencyDownloadCookie.ManifestElement is System.Deployment.Application.Manifest.File)
				{
			...
			{
				throw new InvalidDeploymentException(ExceptionTypes.RefDefValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_DownloadRefDefMismatch"), new object[]
				{
					fileName
				}));
			}
			...
			{
				throw new InvalidDeploymentException(ExceptionTypes.ManifestSemanticValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_AppPrivAsmIdSameAsDeployOrApp"), new object[]
				{
					assemblyManifest.Identity.ToString()
				}));
			}
