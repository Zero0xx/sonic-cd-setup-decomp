...
namespace System.Deployment.Application
{
	...
	internal class ApplicationActivator
	{
		...
		private void DisplayActivationFailureReason(Exception exception, string errorPageUrl)
		{
			...
			this._ui.ShowError(Resources.GetString("UI_ErrorTitle"), message, logFileLocation, text, @string);
		}
		...
		private void ActivateDeploymentWorker(object state)
		{
			...
			try
			{
				...
				try
				{
					...
					Logger.AddPhaseInformation(Resources.GetString("PhaseLog_StartOfActivation"), new object[]
					{
						text
					});
					...
					Logger.AddPhaseInformation(Resources.GetString("ActivateManifestSucceeded"), new object[]
					{
						text
					});
				}
				...
				{
					Logger.AddErrorInformation(ex, Resources.GetString("ActivateManifestException"), new object[]
					{
						text
					});
					if (this._ui == null)
				...
				{
					Logger.AddErrorInformation(exception, Resources.GetString("ActivateManifestException"), new object[]
					{
						text
					});
				}
				...
				{
					Logger.AddErrorInformation(exception2, Resources.GetString("ActivateManifestException"), new object[]
					{
						text
					});
				}
				...
				{
					Logger.AddErrorInformation(ex2, Resources.GetString("ActivateManifestException"), new object[]
					{
						text
					});
					if (ex2.SubType != ExceptionTypes.ActivationInProgress)
					{
						...
						if (!this._ui.SplashCancelled())
						{
							if (ex2.SubType == ExceptionTypes.ActivationLimitExceeded)
							{
								...
								{
									this.DisplayActivationFailureReason(ex2, errorPageUrl);
				...
				catch (Exception ex3)
				{
					...
					Logger.AddErrorInformation(ex3, Resources.GetString("ActivateManifestException"), new object[]
					{
						text
					});
					if (this._ui == null)
					...
					if (!this._ui.SplashCancelled())
					{
						this.DisplayActivationFailureReason(ex3, errorPageUrl);
					}
				}
			}
			finally
		...
		private void PerformDeploymentActivation(Uri activationUri, bool isShortcut, string textualSubId, string deploymentProviderUrlFromExtension, ApplicationActivator.BrowserSettings browserSettings, ref string errorPageUrl)
		{
			...
			try
			{
				...
				else
				{
					...
					AssemblyManifest assemblyManifest = DownloadManager.DownloadDeploymentManifestBypass(this._subStore, ref manifestUri, out tempFile, out subscriptionState, null, null);
					...
					activationDescription.SetDeploymentManifest(assemblyManifest, manifestUri, text);
		...
		private ActivationDescription ProcessOrFollowExtension(Uri associatedFile, string textualSubId, string deploymentProviderUrlFromExtension, ref string errorPageUrl, out TempFile deployFile)
		{
			...
			else
			{
				...
				{
					throw new DeploymentException(ExceptionTypes.Activation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_FileAssociationNoDpUrl"), new object[]
					{
						textualSubId
					}));
				}
				Uri manifestUri = new Uri(deploymentProviderUrlFromExtension);
				AssemblyManifest assemblyManifest = DownloadManager.DownloadDeploymentManifest(this._subStore, ref manifestUri, out deployFile);
				if (assemblyManifest.Description != null)
				{
					errorPageUrl = assemblyManifest.Description.ErrorReportUrl;
				}
				...
				activationDescription = new ActivationDescription();
				activationDescription.SetDeploymentManifest(assemblyManifest, manifestUri, deployFile.Path);
				activationDescription.IsUpdate = false;
		...
		private ActivationDescription ProcessOrFollowShortcut(string shortcutFile, ref string errorPageUrl, out TempFile deployFile)
		{
			...
			Uri uri;
			ShellExposure.ParseAppShortcut(shortcutFile2, out subId, out uri);
			SubscriptionState subscriptionState = this._subStore.GetSubscriptionState(subId);
			ActivationDescription activationDescription = null;
			if (subscriptionState.IsInstalled && subscriptionState.IsShellVisible)
			{
				...
				{
					this.Activate(subscriptionState.CurrentBind, subscriptionState.CurrentApplicationManifest, text, true);
				}
			}
			else
			{
		...
		private void PerformDeploymentUpdate(ref SubscriptionState subState, ref string errorPageUrl)
		{
			...
			if (flag || (subState.PendingDeployment != null && !ApplicationActivator.SkipUpdate(subState, subState.PendingDeployment)))
			{
				...
				try
				{
					...
					{
						Logger.AddErrorInformation(exception, Resources.GetString("Upd_UpdateCheckDownloadFailed"), new object[]
						{
		...
		private void CheckDeploymentProviderValidity(ActivationDescription actDesc, SubscriptionState subState)
		{
			if (actDesc.DeployManifest.Deployment.Install && actDesc.DeployManifest.Deployment.ProviderCodebaseUri == null && subState != null && subState.DeploymentProviderUri != null)
			{
				...
				{
					throw new DeploymentException(ExceptionTypes.DeploymentUriDifferent, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ErrorMessage_DeploymentUriDifferent"), new object[]
					{
						actDesc.DeployManifest.Description.FilteredProduct
					}), new DeploymentException(ExceptionTypes.DeploymentUriDifferent, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_DeploymentUriDifferentExText"), new object[]
					{
						actDesc.DeployManifest.Description.FilteredProduct,
		...
		private void ConsumeUpdatedDeployment(ref SubscriptionState subState, ActivationDescription actDesc)
		{
			...
			if (!actDesc.IsRequiredUpdate)
			{
				...
				if (userInterfaceModalResult == UserInterfaceModalResult.Skip)
				{
					...
					DateTime updateSkipTime = DateTime.UtcNow + t;
		...
		private bool InstallApplication(ref SubscriptionState subState, ActivationDescription actDesc)
		{
			...
			using (this._subStore.AcquireReferenceTransaction(out transactionId))
			{
				...
				try
				{
					result = this.DownloadApplication(subState, actDesc, transactionId, out tempDirectory);
		...
		private bool DownloadApplication(SubscriptionState subState, ActivationDescription actDesc, long transactionId, out TempDirectory downloadTemp)
		{
			...
			Uri manifestUri;
			string manifestPath;
			AssemblyManifest assemblyManifest = DownloadManager.DownloadApplicationManifest(actDesc.DeployManifest, downloadTemp.Path, actDesc.DeploySourceUri, out manifestUri, out manifestPath);
			...
			actDesc.SetApplicationManifest(assemblyManifest, manifestUri, manifestPath);
			...
			if (!this._subStore.CheckAndReferenceApplication(subState, actDesc.AppId, transactionId))
			{
				...
				if (actDesc.IsUpdate)
				{
					if (actDesc.IsRequiredUpdate)
					{
						userInterfaceInfo.formTitle = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("UI_ProgressTitleRequiredUpdate"), new object[]
						{
							userInterfaceInfo.productName
						});
					}
					else
					{
						userInterfaceInfo.formTitle = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("UI_ProgressTitleUpdate"), new object[]
						{
							userInterfaceInfo.productName
						});
					}
				...
				{
					userInterfaceInfo.formTitle = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("UI_ProgressTitleDownload"), new object[]
					{
						userInterfaceInfo.productName
					});
				}
				...
				{
					userInterfaceInfo.formTitle = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("UI_ProgressTitleInstall"), new object[]
					{
						userInterfaceInfo.productName
					});
				}
				...
				ProgressPiece notification = this._ui.ShowProgress(userInterfaceInfo);
				DownloadOptions downloadOptions = null;
				bool flag = !actDesc.DeployManifest.Deployment.Install;
				if (!this._fullTrust && flag)
				{
