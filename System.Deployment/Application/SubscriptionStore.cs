	...
	internal class SubscriptionStore
	{
				...
				if (SubscriptionStore._userStore == null)
				{
					lock (SubscriptionStore._currentUserLock)
					{
						if (SubscriptionStore._userStore == null)
						{
							...
							SubscriptionStore._userStore = new SubscriptionStore(deployPath, tempPath, ComponentStoreType.UserStore);
						}
					}
				}
		...
		public void CommitApplication(ref SubscriptionState subState, CommitApplicationParams commitParams)
		{
			using (this.AcquireSubscriptionWriterLock(subState))
			{
				...
				{
					throw new DeploymentException(ExceptionTypes.GroupMultipleMatch, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_GroupMultipleMatch"), new object[]
					{
						text
					}));
				}
		...
		public void CheckForDeploymentUpdate(SubscriptionState subState)
		{
			...
			try
			{
				...
				Version v = this.CheckUpdateInManifest(subState, deploymentProviderUri, assemblyManifest, subState.CurrentDeployment.Version);
				DefinitionIdentity deployId = (v != null) ? assemblyManifest.Identity : null;
				this.SetPendingDeployment(subState, deployId, DateTime.UtcNow);
				if (v != null && assemblyManifest.Identity.Equals(subState.PendingDeployment))
				{
		...
		public bool CheckGroupInstalled(SubscriptionState subState, DefinitionAppId appId, string groupName)
		{
			...
			using (this.AcquireSubscriptionReaderLock(subState))
			{
				result = this._compStore.CheckGroupInstalled(appId, groupName);
			}
			return result;
		}
		...
		public bool CheckGroupInstalled(SubscriptionState subState, DefinitionAppId appId, AssemblyManifest appManifest, string groupName)
		{
			...
			using (this.AcquireSubscriptionReaderLock(subState))
			{
				result = this._compStore.CheckGroupInstalled(appId, appManifest, groupName);
			}
			return result;
		}
		...
		internal Store.IPathLock LockApplicationPath(DefinitionAppId definitionAppId)
		{
			...
			using (this.AcquireStoreReaderLock())
			{
				result = this._compStore.LockApplicationPath(definitionAppId);
			}
			return result;
		}
		...
		private void CheckApplicationPayload(CommitApplicationParams commitParams)
		{
			...
			{
				byte[] array = ComponentVerifier.GenerateDigestValue(text, CMS_HASH_DIGESTMETHOD.CMS_HASH_DIGESTMETHOD_SHA1, CMS_HASH_TRANSFORM.CMS_HASH_TRANSFORM_IDENTITY);
		...
		private bool IsAssemblyInstalled(DefinitionIdentity asmId)
		{
			...
			using (this.AcquireStoreReaderLock())
			{
				result = this._compStore.IsAssemblyInstalled(asmId);
			}
			return result;
		}
