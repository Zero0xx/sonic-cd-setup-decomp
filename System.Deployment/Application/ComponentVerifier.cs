...
namespace System.Deployment.Application
{
	...
	internal class ComponentVerifier
	{
		...
		{
			ComponentVerifier.FileComponent value = new ComponentVerifier.FileComponent(filePath, verificationHashCollection);
			this._verificationComponents.Add(value);
		}
		...
		{
			ComponentVerifier.SimplyNamedAssemblyComponent value = new ComponentVerifier.SimplyNamedAssemblyComponent(filePath, assemblyManifest);
			this._verificationComponents.Add(value);
		}
		...
		{
			ComponentVerifier.StrongNameAssemblyComponent value = new ComponentVerifier.StrongNameAssemblyComponent(filePath, assemblyManifest);
			this._verificationComponents.Add(value);
		}
		...
		public static void VerifyFileHash(string filePath, HashCollection hashCollection)
		{
			...
			if (hashCollection.Count == 0)
			{
				...
				{
					throw new InvalidDeploymentException(ExceptionTypes.HashValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_HashNotSpecified"), new object[]
					{
						fileName
					}));
				}
				Logger.AddWarningInformation(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("NoHashFile"), new object[]
				{
					fileName
				}));
			}
		...
		public static void VerifyFileHash(string filePath, Hash hash)
		{
			...
			if (!flag)
			{
				throw new InvalidDeploymentException(ExceptionTypes.HashValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_DifferentHashes"), new object[]
				{
					fileName
				}));
			}
		...
		public static HashAlgorithm GetHashAlgorithm(CMS_HASH_DIGESTMETHOD digestMethod)
		{
			...
			throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_DigestMethodNotSupported"), new object[]
			{
		...
		public static Stream GetTransformedStream(string filePath, CMS_HASH_TRANSFORM transform)
		{
			...
			{
				throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_TransformAlgorithmNotSupported"), new object[]
				{
					transform.ToString()
				}));
			}
		...
		public static void VerifySimplyNamedAssembly(string filePath, AssemblyManifest assemblyManifest)
		{
			...
			{
				throw new InvalidDeploymentException(ExceptionTypes.Validation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_SimplyNamedAsmWithPKT"), new object[]
				{
					fileName
				}));
			}
			...
			{
				throw new InvalidDeploymentException(ExceptionTypes.IdentityMatchValidationForMixedModeAssembly, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_SimplyNamedAsmWithStrongNameComplib"), new object[]
				{
					fileName
				}));
			}
		...
		public static void VerifyStrongNameAssembly(string filePath, AssemblyManifest assemblyManifest)
		{
			...
			if (assemblyManifest.Identity.PublicKeyToken == null)
			{
				throw new InvalidDeploymentException(ExceptionTypes.Validation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_StrongNameAsmWithNoPKT"), new object[]
				{
					fileName
				}));
			}
			...
			else if (assemblyManifest.ManifestSourceFormat == ManifestSourceFormat.ID_1)
			{
				if (assemblyManifest.ComplibIdentity == null)
				{
					...
					try
					{
						...
						throw new InvalidDeploymentException(ExceptionTypes.StronglyNamedAssemblyVerification, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_StronglyNamedAssemblyNotVerifiable"), new object[]
						{
							fileName
						}));
					}
				...
				{
					throw new InvalidDeploymentException(ExceptionTypes.IdentityMatchValidationForMixedModeAssembly, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_IdentitiesDoNotMatchForMixedModeAssembly"), new object[]
					{
						fileName
					}));
				}
				...
				{
					throw new InvalidDeploymentException(ExceptionTypes.SignatureValidation, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_StrongNameSignatureInvalid"), new object[]
					{
						fileName
					}));
				}
