...
namespace System.Deployment.Application
{
	...
	internal static class ShellExposure
	{
		...
		private static void AddFileAssociation(FileAssociation fileAssociation, DefinitionIdentity subId, Uri deploymentProviderUri)
		{
			...
			{
				Logger.AddWarningInformation(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("SkippedFileAssoc"), new object[]
				{
				...
				if (registryKey != null)
				{
					...
					if (value is string)
					{
						...
						if (string.Equals(a, subId.ToString(), StringComparison.Ordinal))
						{
							...
							{
								throw new DeploymentException(ExceptionTypes.InvalidARPEntry, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_FileAssocExtDeleteFailed"), new object[]
								{
									fileAssociation.Extension,
									productName
								}), innerException);
		...
		private static string RemoveFileAssociationProgIDInfo(FileAssociation fileAssociation, DefinitionIdentity subId, RegistryKey classesKey, string productName)
		{
			...
			using (RegistryKey registryKey = classesKey.OpenSubKey(fileAssociation.ProgID, true))
			{
				...
				result = (string)registryKey.GetValue("Guid");
				try
				{
				...
				{
					throw new DeploymentException(ExceptionTypes.InvalidARPEntry, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_FileAssocProgIdDeleteFailed"), new object[]
					{
						fileAssociation.ProgID,
						productName
					}), innerException);
				}
			}
					...
					using (RegistryKey registryKey2 = registryKey.OpenSubKey(clsIdString))
					{
						...
						if (value is string)
						{
							...
							if (string.Equals(a, subId.ToString(), StringComparison.Ordinal))
							{
								...
								{
									throw new DeploymentException(ExceptionTypes.InvalidARPEntry, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_FileAssocCLSIDDeleteFailed"), new object[]
									{
										clsIdString,
										productName
									}), innerException);
		...
		{
			DefinitionIdentity subscriptionId = subState.SubscriptionId;
			string text = string.Format(CultureInfo.InvariantCulture, "rundll32.exe dfshim.dll,ShArpMaintain {0}", new object[]
			{
		...
		private static void RemoveArpEntry(DefinitionIdentity subId)
		{
			using (RegistryKey registryKey = ShellExposure.UninstallRoot.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall", true))
			{
				...
				{
					throw new DeploymentException(ExceptionTypes.InvalidARPEntry, string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_ArpEntryRemovalFailure"), new object[]
					{
						text
					}), innerException);
				}
		...
		{
			return string.Format(CultureInfo.InvariantCulture, "{0:x16}", new object[]
			{
				subId.Hash
			});
		}
		...
		public class ShellExposureInformation
		{
					...
					if (registryKey != null)
					{
						using (RegistryKey registryKey2 = registryKey.OpenSubKey(ShellExposure.GenerateArpKeyName(subscriptionIdentity)))
						{
							if (registryKey2 != null)
							{
								...
								{
									shortcutAppId = (registryKey2.GetValue("ShortcutAppId") as string);
								}
								else
								{
									shortcutAppId = "";
			...
			public static ShellExposure.ShellExposureInformation CreateShellExposureInformation(string publisher, string suiteName, string product, string shortcutAppId)
			{
				...
				for (int num = 0; num != 2147483647; num++)
				{
					...
					if (num == 0)
					{
						text3 = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ShellExposure_DisplayStringNoIndex"), new object[]
						{
							product
						});
					}
					...
					{
						text3 = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("ShellExposure_DisplayStringWithIndex"), new object[]
						{
							product,
							num
						});
					...
					if (!System.IO.File.Exists(text4) && !System.IO.File.Exists(text5))
					{
						...
						shellExposureInformation._appSupportShortcut = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("SupportUrlFormatter"), new object[]
						{
							text3
						});
