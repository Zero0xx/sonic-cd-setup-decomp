...
namespace System.Deployment.Application
{
	...
	public sealed class ApplicationDeployment
	{
		...
		private ApplicationDeployment(string fullAppId)
		{
			...
			{
				throw new InvalidDeploymentException(string.Format(CultureInfo.CurrentUICulture, Resources.GetString("Ex_SubAppIdNotValid"), new object[]
				{
					fullAppId
				}), innerException);
			}
		...
		private void CheckForUpdateBindCompletedEventHandler(object sender, BindCompletedEventArgs e)
		{
			...
			finally
			{
				...
				{
					checkForUpdateCompletedEventHandler(this, e2);
				}
		...
		private void UpdateBindCompletedEventHandler(object sender, BindCompletedEventArgs e)
		{
			...
			finally
			{
				...
				if (!flag)
				{
					this.EndUpdateAsync(deploymentManager, error, e.Cancelled);
				}
			}
		}
		...
		private void SynchronizeGroupCompletedEventHandler(object sender, SynchronizeCompletedEventArgs e)
		{
			...
			finally
			{
				...
				{
					downloadFileGroupCompletedEventHandler(this, e2);
				}
			}
		}
		...
		private UpdateCheckInfo DetermineUpdateCheckResult(ActivationDescription actDesc)
		{
			...
			this._subState.Invalidate();
			Version version2 = this._subStore.CheckUpdateInManifest(this._subState, actDesc.DeploySourceUri, deployManifest, this._currentVersion);
			if (version2 != null && !deployManifest.Identity.Equals(this._subState.ExcludedDeployment))
			{
				updateAvailable = true;
				availableVersion = version2;
				version = deployManifest.Deployment.MinimumRequiredVersion;
				if (version != null && version.CompareTo(this._currentVersion) > 0)
				{
