...
namespace System.Deployment.Application
{
	...
	internal class TempFile : DisposableBase
	{
		...
		public TempFile(string basePath, string suffix)
		{
			do
			{
				this._thePath = System.IO.Path.Combine(basePath, PathHelper.GenerateRandomPath(2U) + suffix);
			}
			while (File.Exists(this._thePath) || Directory.Exists(this._thePath));
			string directoryName = System.IO.Path.GetDirectoryName(this._thePath);
