...
namespace System.Deployment.Application
{
	...
	public class DeploymentServiceCom
	{
			...
			using (UserInterface userInterface = new UserInterface())
			{
				...
				try
				{
					...
					try
					{
						...
						userInterfaceInfo.formTitle = string.Format(CultureInfo.CurrentUICulture, Resources.GetString("UI_MaintenanceTitle"), new object[]
						{
				...
				{
					Logger.AddErrorInformation(ex2, Resources.GetString(array[0]), new object[]
					{
						textualSubId
					});
					ex = ex2;
				...
				{
					Logger.AddPhaseInformation(Resources.GetString(flag ? array[1] : array[2]), new object[]
					{
						textualSubId
					});
		...
		private SubscriptionState GetSubscriptionState(string textualSubId)
		{
			...
			{
				throw new DeploymentException(ExceptionTypes.SubscriptionState, string.Format(CultureInfo.CurrentCulture, Resources.GetString("Ex_ComArgSubIdentityNotValid"), new object[]
				{
					textualSubId
				}), innerException);
			}
