...
namespace System.Xml.Xsl.XPath
{
	...
	internal class XPathParser<Node>
	{
		...
		internal static XPathAxis GetAxis(string axisName, XPathScanner scanner)
		{
			...
			throw scanner.CreateException("XPath_UnknownAxis", new string[]
			{
				axisName
			});
		}
		...
		private Node ParseStep()
		{
			...
			else
			{
				...
				else
				{
					...
					{
						axis = XPathParser<Node>.GetAxis(this.scanner.Name, this.scanner);
						this.scanner.NextLex();
						goto IL_135;
					}
				...
				goto IL_135;
				IL_10E:
				throw this.scanner.CreateException("XPath_UnexpectedToken", new string[]
				{
		...
		internal static void InternalParseNodeTest(XPathScanner scanner, XPathAxis axis, out XPathNodeType nodeType, out string nodePrefix, out string nodeName)
		{
			...
			if (kind != LexKind.Name)
			{
				throw scanner.CreateException("XPath_NodeTestExpected", new string[]
				{
					scanner.RawValue
				});
			}
		...
		private Node ParseEqualityExpr()
		{
			...
			while ((flag = (this.scanner.Kind == LexKind.Eq)) || this.scanner.Kind == LexKind.Ne)
			{
				...
				this.scanner.NextLex();
				node = this.builder.Operator(op, node, this.ParseRelationalExpr());
			}
			return node;
		}
		...
		private Node ParseRelationalExpr()
		{
			...
			for (;;)
			{
				...
				this.scanner.NextLex();
				node = this.builder.Operator(op, node, this.ParseAdditiveExpr());
			}
			return node;
		}
		...
		private Node ParseAdditiveExpr()
		{
			...
			while ((flag = (this.scanner.Kind == LexKind.Plus)) || this.scanner.Kind == LexKind.Minus)
			{
				...
				this.scanner.NextLex();
				node = this.builder.Operator(op, node, this.ParseMultiplicativeExpr());
			}
			return node;
		}
		...
		private Node ParseMultiplicativeExpr()
		{
			...
			for (;;)
			{
				...
				this.scanner.NextLex();
				node = this.builder.Operator(op, node, this.ParseUnaryExpr());
			}
			return node;
		}
		...
		private Node ParsePrimaryExpr()
		{
			...
			else
			{
				...
				{
					result = this.builder.Number(this.scanner.NumberValue);
					this.scanner.NextLex();
					return result;
				}
