	...
	internal class Compiler
	{
		...
		internal int InsertVariable(VariableAction variable)
		{
			...
			if (variableAction != null)
			{
				...
				{
					throw XsltException.Create("Xslt_DupVarName", new string[]
					{
						variable.NameStr
					});
				}
		...
		internal string GetSingleAttribute(string attributeAtom)
		{
			...
			if (navigatorInput.MoveToFirstAttribute())
			{
				...
				throw XsltException.Create("Xslt_InvalidAttribute", new string[]
				{
					localName2,
					localName
				});
			...
			if (text == null)
			{
				throw XsltException.Create("Xslt_MissingAttribute", new string[]
				{
					attributeAtom
				});
			}
		...
		internal Uri ResolveUri(string relativeUri)
		{
			...
			if (uri == null)
			{
				throw XsltException.Create("Xslt_CantResolve", new string[]
				{
					relativeUri
				});
			}
		...
		internal int AddQuery(string xpathQuery, bool allowVar, bool allowKey, bool isPattern)
		{
			...
			catch (XPathException inner)
			{
				if (!this.ForwardCompatibility)
				{
					throw XsltException.Create("Xslt_InvalidXPath", new string[]
					{
						xpathQuery
					}, inner);
				}
		...
		internal void AddScript(string source, ScriptingLanguage lang, string ns, string fileName, int lineNumber)
		{
			...
			for (ScriptingLanguage scriptingLanguage = ScriptingLanguage.JScript; scriptingLanguage <= ScriptingLanguage.CSharp; scriptingLanguage++)
			{
				...
				{
					throw XsltException.Create("Xslt_ScriptMixedLanguages", new string[]
					{
						ns
					});
				}
		...
		private void CompileAssembly(ScriptingLanguage lang, Hashtable typeDecls, string nsName, Evidence evidence)
		{
			...
			if (compilerResults.Errors.HasErrors)
			{
				...
				throw XsltException.Create("Xslt_ScriptCompileErrors", new string[]
				{
		...
		private static void getTextLex(string avt, ref int start, StringBuilder lex)
		{
			...
			for (i = start; i < length; i++)
			{
				...
				else if (c == '}')
				{
					if (i + 1 >= length || avt[i + 1] != '}')
					{
						throw XsltException.Create("Xslt_SingleRightAvt", new string[]
						{
							avt
						});
					}
		...
		private static void getXPathLex(string avt, ref int start, StringBuilder lex)
		{
				...
				case 0:
				{
					...
					if (c2 != '"')
					{
						if (c2 != '\'')
						{
							...
							{
							case '{':
								throw XsltException.Create("Xslt_NestedAvt", new string[]
								{
									avt
								});
							case '}':
								...
								if (i == start + 2)
								{
									throw XsltException.Create("Xslt_EmptyAvtExpr", new string[]
									{
										avt
									});
								}
			...
			throw XsltException.Create((num == 0) ? "Xslt_OpenBracesAvt" : "Xslt_OpenLiteralAvt", new string[]
			{
				avt
			});
		}
		...
		public XsltException UnexpectedKeyword()
		{
			...
			return XsltException.Create("Xslt_UnexpectedKeyword", new string[]
			{
				name,
				name2
			});
		...
		internal class ErrorXPathExpression : CompiledXpathExpr
		{
			...
			{
				throw new XsltException("Xslt_InvalidXPath", new string[]
				{
					this.Expression
