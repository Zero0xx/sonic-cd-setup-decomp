...
namespace System.Xml.Xsl.XsltOld
{
	...
	internal class ContainerAction : CompiledAction
	{
		...
		private void CompileInclude(Compiler compiler)
		{
			...
			{
				throw XsltException.Create("Xslt_CircularInclude", new string[]
				{
					text
				});
			}
		...
		protected void CompileTopLevelElements(Compiler compiler)
		{
			...
			Block_23:
			throw XsltException.Create("Xslt_NullNsAtTopLevel", new string[]
			{
				input.Name
			});
			IL_2F4:
		...
		private void AddScript(Compiler compiler)
		{
			...
			if (input.MoveToFirstAttribute())
			{
				...
				throw XsltException.Create("Xslt_ScriptInvalidLanguage", new string[]
				{
					value
				});
				Block_6:
		...
		internal void CheckDuplicateParams(XmlQualifiedName name)
		{
			if (this.containedActions != null)
			{
				foreach (object obj in this.containedActions)
				{
					...
					{
						throw XsltException.Create("Xslt_DuplicateWithParam", new string[]
						{
							name.ToString()
						});
					}
