...
namespace System.Xml.Xsl.XsltOld
{
	...
	internal class XsltCompileContext : XsltContext
	{
		...
		public override IXsltContextVariable ResolveVariable(string prefix, string name)
		{
			...
			{
				throw XsltException.Create("Xslt_InvalidVariable", new string[]
				{
		...
		public override IXsltContextFunction ResolveFunction(string prefix, string name, XPathResultType[] argTypes)
		{
			...
			else
			{
				...
				else
				{
					...
					if (obj == null)
					{
						throw XsltException.Create("Xslt_ScriptInvalidPrefix", new string[]
						{
							prefix
						});
					}
			...
			{
				throw XsltException.Create("Xslt_UnknownXsltFunction", new string[]
				{
					name
				});
			}
		...
		private static void EvaluateKey(XPathNavigator node, Query matchExpr, string matchStr, Query useExpr, Hashtable keyTable)
		{
			...
			{
				throw XsltException.Create("Xslt_InvalidPattern", new string[]
				{
					matchStr
				});
			}
		...
		private DecimalFormat ResolveFormatName(string formatName)
		{
			...
			if (decimalFormat == null)
			{
				if (formatName != null)
				{
					throw XsltException.Create("Xslt_NoDecimalFormat", new string[]
					{
						formatName
					});
				}
		...
		private bool ElementAvailable(string qname)
		{
			...
			string a2 = this.manager.ResolveXmlNamespace(prefix);
			return a2 == "http://www.w3.org/1999/XSL/Transform" && (a == "apply-imports" || a == "apply-templates" || a == "attribute" || a == "call-template" || a == "choose" || a == "comment" || a == "copy" || a == "copy-of" || a == "element" || a == "fallback" || a == "for-each" || a == "if" || a == "message" || a == "number" || a == "processing-instruction" || a == "text" || a == "value-of" || a == "variable");
		}
		...
		private string SystemProperty(string qname)
		{
			...
			{
				throw XsltException.Create("Xslt_InvalidPrefix", new string[]
				{
					text
				});
			}
		...
		private class FuncUnEntityUri : XsltCompileContext.XsltFunctionImpl
		{
			...
			public override object Invoke(XsltContext xsltContext, object[] args, XPathNavigator docContext)
			{
				throw XsltException.Create("Xslt_UnsuppFunction", new string[]
				{
					"unparsed-entity-uri"
				});
			}
