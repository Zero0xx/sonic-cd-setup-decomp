	...
	public static class XsltFunctions
	{
			...
			try
			{
				...
				else
				{
					...
					if (isDate)
					{
						...
						{
							int dateFormat = XsltFunctions.GetDateFormat(lcid, 0U, ref systemTime, format, stringBuilder, 0);
							if (dateFormat != 0)
							{
								stringBuilder = new StringBuilder(dateFormat);
								dateFormat = XsltFunctions.GetDateFormat(lcid, 0U, ref systemTime, format, stringBuilder, stringBuilder.Capacity);
							}
						}
					}
					else if (XsltFunctions.GetTimeFormat(lcid, 0U, ref systemTime, format, stringBuilder, stringBuilder.Capacity) == 0)
					{
						int timeFormat = XsltFunctions.GetTimeFormat(lcid, 0U, ref systemTime, format, stringBuilder, 0);
						if (timeFormat != 0)
						{
							stringBuilder = new StringBuilder(timeFormat);
							timeFormat = XsltFunctions.GetTimeFormat(lcid, 0U, ref systemTime, format, stringBuilder, stringBuilder.Capacity);
						}
					}
		...
		public static double MSStringCompare(string s1, string s2, string lang, string options)
		{
			...
			if (flag)
			{
				...
				{
					throw new XslTransformException("Xslt_InvalidCompareOption", new string[]
					{
						options
					});
				}
		...
		{
			int length;
			int num = ValidateNames.ParseQName(name, 0, out length);
			if (num != name.Length)
			{
				return string.Empty;
			}
		...
		private static CultureInfo GetCultureInfo(string lang)
		{
			...
			{
				throw new XslTransformException("Xslt_InvalidLanguage", new string[]
				{
					lang
				});
			}
