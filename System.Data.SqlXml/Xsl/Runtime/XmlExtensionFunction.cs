...
namespace System.Xml.Xsl.Runtime
{
	...
	internal class XmlExtensionFunction
	{
		...
		public void Bind()
		{
			...
			if (methodInfo == null)
			{
				...
				foreach (MethodInfo methodInfo3 in methods)
				{
					...
					{
						throw new XslTransformException("XmlIl_NonPublicExtensionMethod", new string[]
						{
							this.namespaceUri,
							this.name
						});
			...
			{
				throw new XslTransformException("XmlIl_GenericExtensionMethod", new string[]
				{
					this.namespaceUri,
					this.name
				});
		...
		public object Invoke(object extObj, object[] args)
		{
			...
			{
				throw new XslTransformException(ex.InnerException, "XmlIl_ExtensionError", new string[]
				{
					this.name
				});
			}
			catch (Exception ex2)
			{
				...
				throw new XslTransformException(ex2, "XmlIl_ExtensionError", new string[]
				{
					this.name
				});
			}
		...
		private Type GetClrType(Type clrType)
		{
			...
			if (clrType.IsByRef)
			{
				throw new XslTransformException("XmlIl_ByRefType", new string[]
				{
					this.namespaceUri,
					this.name
				});
