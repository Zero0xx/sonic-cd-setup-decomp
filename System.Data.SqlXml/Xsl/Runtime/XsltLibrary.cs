...
namespace System.Xml.Xsl.Runtime
{
	...
	public sealed class XsltLibrary
	{
		...
		public int CheckScriptNamespace(string nsUri)
		{
			...
			{
				throw new XslTransformException("Xslt_ScriptAndExtensionClash", new string[]
				{
					nsUri
				});
			}
		...
		public string FormatNumberDynamic(double value, string formatPicture, XmlQualifiedName decimalFormatName, string errorMessageName)
		{
			...
			else
			{
				...
				{
					throw new XslTransformException("Xslt_NoDecimalFormat", new string[]
					{
						errorMessageName
					});
				}
			...
			if (lang != null)
			{
				if (lang.Length == 0)
				{
					if (!forwardCompatibility)
					{
						if (errorHelper == null)
						{
							throw new XslTransformException("Xslt_InvalidAttrValue", new string[]
							{
								"lang",
								lang
							});
						}
						errorHelper.ReportError("Xslt_InvalidAttrValue", new string[]
						{
							"lang",
							lang
						});
				...
				else
				{
					...
					catch (ArgumentException)
					{
						if (!forwardCompatibility)
						{
							...
							errorHelper.ReportError("Xslt_InvalidLanguage", new string[]
							{
								lang
							});
						}
