	...
	internal sealed class QilXmlReader
	{
		...
		private void EndElement()
		{
			...
			case "XsltInvokeEarlyBound":
			{
				...
				qilNode = this.f.XsltInvokeEarlyBound(qilName, this.f.LiteralObject(value), qilList[1], readerAnnotation.XmlType);
				goto IL_605;
			}
			}
			MethodInfo methodInfo = QilXmlReader.nameToFactoryMethod[this.r.LocalName];
			object[] array = new object[qilList.Count];
			for (int k = 0; k < array.Length; k++)
			{
				array[k] = qilList[k];
			}
			qilNode = (QilNode)methodInfo.Invoke(this.f, array);
			IL_605:
		...
		private QilName ParseName(string name)
		{
			if (name != null && name.Length > 0)
			{
				...
				return this.f.LiteralQName(localName, namespaceUri, prefix);
			}
			return null;
		}
