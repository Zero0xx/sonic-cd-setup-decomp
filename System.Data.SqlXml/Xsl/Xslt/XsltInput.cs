...
namespace System.Xml.Xsl.Xslt
{
	...
	internal class XsltInput : IErrorHelper
	{
		...
		public XsltInput.ContextInfo GetAttributes(int required, int number, string[] names, string[] values)
		{
			...
			while (this.MoveToNextAttOrNs())
			{
				...
				else
				{
					...
					{
						this.ReportError("Xslt_InvalidAttribute", new string[]
						{
							this.QualifiedName,
							qualifiedName
						});
			...
			for (int k = 0; k < required; k++)
			{
				if (values[k] == null)
				{
					this.ReportError("Xslt_MissingAttribute", new string[]
					{
						names[k]
					});
				}
		...
		public ISourceLineInfo BuildLineInfo()
		{
			...
			SourceLineInfo result = new SourceLineInfo(this.Uri, this.StartLine, this.StartPos, this.EndLine, this.EndPos);
			if (!this.OnTextNode && !flag)
			{
				this.lastLineInfo = result;
			}
			return result;
		}
		...
		private class InputScopeManager
		{
			...
			private void AddRecord()
			{
				...
				{
					XsltInput.InputScopeManager.ScopeRecord[] destinationArray = new XsltInput.InputScopeManager.ScopeRecord[this.lastRecord * 2];
