...
namespace System.Xml.Xsl.Xslt
{
	...
	internal class Scripts
	{
		...
		public ScriptClass GetScriptClass(string ns, string language, IErrorHelper errorHelper)
		{
			...
			{
				errorHelper.ReportError("Xslt_ScriptInvalidLanguage", new string[]
				{
					language
				});
				return null;
			...
			foreach (ScriptClass scriptClass in this.scriptClasses)
			{
				if (ns == scriptClass.ns)
				{
					...
					{
						errorHelper.ReportError("Xslt_ScriptMixedLanguages", new string[]
						{
							ns
						});
						return null;
		...
		private Assembly CompileAssembly(List<ScriptClass> scriptsForLang)
		{
			...
			bool keepFiles = ((this.compiler.IsDebug && text == null) || XmlILTrace.IsEnabled) && !settings.CheckOnly;
			...
			foreach (object obj2 in compilerResults.Errors)
			{
				...
				Scripts.FixErrorPosition(error, scriptsForLang);
			}
