		...
		private void LoadPicture(UnsafeNativeMethods.IStream stream)
		{
			...
			try
			{
				...
				try
				{
					...
					Size iconSize = this.GetIconSize(iconHandle);
					this.handle = SafeNativeMethods.CopyImageAsCursor(new HandleRef(this, iconHandle), 2, iconSize.Width, iconSize.Height, 0);
					this.ownHandle = true;
				}
				finally
				{
