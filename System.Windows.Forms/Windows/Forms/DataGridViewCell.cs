...
namespace System.Windows.Forms
{
	...
	public abstract class DataGridViewCell : DataGridViewElement, ICloneable, IDisposable
	{
		...
		internal void ComputeBorderStyleCellStateAndCellBounds(int rowIndex, out DataGridViewAdvancedBorderStyle dgvabsEffective, out DataGridViewElementStates cellState, out Rectangle cellBounds)
		{
			...
			{
				dgvabsEffective = this.AdjustCellBorderStyle(base.DataGridView.AdvancedCellBorderStyle, dataGridViewAdvancedBorderStylePlaceholder, singleVerticalBorderAdded, singleHorizontalBorderAdded, rowIndex == base.DataGridView.FirstDisplayedRowIndex, this.ColumnIndex == base.DataGridView.FirstDisplayedColumnIndex);
		...
		internal Rectangle ComputeErrorIconBounds(Rectangle cellValueBounds)
		{
			...
			{
				Rectangle result = new Rectangle(base.DataGridView.RightToLeftInternal ? (cellValueBounds.Left + 4) : (cellValueBounds.Right - 4 - 12), cellValueBounds.Y + (cellValueBounds.Height - 11) / 2, 12, 11);
				return result;
			}
			return Rectangle.Empty;
		}
