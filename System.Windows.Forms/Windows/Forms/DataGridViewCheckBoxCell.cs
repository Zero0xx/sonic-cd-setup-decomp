	...
	public class DataGridViewCheckBoxCell : DataGridViewCell, IDataGridViewEditingCell
	{
		...
		protected override Size GetPreferredSize(Graphics graphics, DataGridViewCellStyle cellStyle, int rowIndex, Size constraintSize)
		{
			...
			if (base.DataGridView.ShowCellErrors)
			{
				...
				{
					result.Width = Math.Max(result.Width, num + 8 + 12);
				}
				if (freeDimensionFromConstraint != DataGridViewFreeDimension.Width)
				{
					result.Height = Math.Max(result.Height, num2 + 8 + 11);
				}
			}
			return result;
		}
				...
				else
				{
					...
					if ((this.ButtonState & (ButtonState.Checked | ButtonState.Pushed)) != ButtonState.Normal)
					{
						...
						if (paint && DataGridViewCell.PaintContentForeground(paintParts))
						{
							...
							CheckBoxBaseAdapter.DrawCheckOnly(glyphSize.Width, checkState == CheckState.Checked || checkState == CheckState.Indeterminate, base.DataGridView.Enabled, checkState, g, layoutData, colorData, colorData.windowText, colorData.buttonFace, true);
						}
					...
					else if (base.DataGridView.MouseEnteredCellAddress.Y == rowIndex && base.DataGridView.MouseEnteredCellAddress.X == base.ColumnIndex && DataGridViewCheckBoxCell.mouseInContentBounds)
					{
						...
						if (paint && DataGridViewCell.PaintContentForeground(paintParts))
						{
							...
							CheckBoxBaseAdapter.DrawCheckOnly(glyphSize.Width, checkState == CheckState.Checked || checkState == CheckState.Indeterminate, base.DataGridView.Enabled, checkState, g, layoutData2, colorData2, colorData2.windowText, colorData2.highlight, true);
						}
					...
					else
					{
						...
						if (paint && DataGridViewCell.PaintContentForeground(paintParts))
						{
							...
							CheckBoxBaseAdapter.DrawCheckOnly(glyphSize.Width, checkState == CheckState.Checked || checkState == CheckState.Indeterminate, base.DataGridView.Enabled, checkState, g, layoutData3, colorData3, colorData3.windowText, colorData3.highlight, true);
						}
				...
				if (!dataGridViewCheckBoxCell.ReadOnly && dataGridViewCheckBoxCell.OwningColumn != null && dataGridViewCheckBoxCell.OwningRow != null)
				{
					...
					if (dataGridView.IsCurrentCellInEditMode)
					{
						if (dataGridViewCheckBoxCell.SwitchFormattedValue())
						{
							...
							int objectID = rowCount + (columnHeadersVisible ? 1 : 0) + 1;
							int childID = num + (rowHeadersVisible ? 1 : 0);
							Control.ControlAccessibleObject controlAccessibleObject = (Control.ControlAccessibleObject)dataGridView.AccessibilityObject;
							controlAccessibleObject.NotifyClients(AccessibleEvents.DefaultActionChange, objectID, childID);
						}
						if (flag)
						{
