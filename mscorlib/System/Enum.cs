	...
	public abstract class Enum : ValueType, IComparable, IFormattable, IConvertible
	{
		...
		private static string InternalGetValueAsString(Type enumType, object value)
		{
			...
			if (underlyingType == Enum.intType || underlyingType == typeof(short) || underlyingType == typeof(long) || underlyingType == typeof(ushort) || underlyingType == typeof(byte) || underlyingType == typeof(sbyte) || underlyingType == typeof(uint) || underlyingType == typeof(ulong))
			{
				...
				int num = Enum.BinarySearch(hashEntry.values, value2);
				if (num >= 0)
				{
		...
		public static object Parse(Type enumType, string value, bool ignoreCase)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (!flag)
				{
					throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Arg_EnumValueNotFound"), new object[]
					{
						value
					}));
				}
		...
		public static Array GetValues(Type enumType)
		{
			...
			{
				object value = Enum.ToObject(enumType, values[i]);
		...
		public static bool IsDefined(Type enumType, object value)
		{
			...
			{
				ulong value2 = Enum.ToUInt64(value);
				return Enum.BinarySearch(values, value2) >= 0;
			}
			throw new InvalidOperationException(Environment.GetResourceString("InvalidOperation_UnknownEnumType"));
		}
		...
		public override string ToString()
		{
			Type type = base.GetType();
			FieldInfo valueField = Enum.GetValueField(type);
			object value = ((RtFieldInfo)valueField).InternalGetValue(this, false);
			return Enum.InternalFormat(type, value);
		}
