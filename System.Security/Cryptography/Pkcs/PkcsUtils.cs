...
namespace System.Security.Cryptography.Pkcs
{
	...
	internal class PkcsUtils
	{
		...
		internal unsafe static int GetMaxKeyLength(SafeCryptProvHandle safeCryptProvHandle, uint algId)
		{
			...
			{
				while (CAPISafe.CryptGetProvParam(safeCryptProvHandle, 22U, safeLocalAllocHandle.DangerousGetHandle(), new IntPtr((void*)(&num)), dwFlags))
				{
		...
		internal unsafe static uint GetVersion(SafeCryptMsgHandle safeCryptMsgHandle)
		{
			...
			if (!CAPISafe.CryptMsgGetParam(safeCryptMsgHandle, 30U, 0U, new IntPtr((void*)(&result)), new IntPtr((void*)(&num))))
			{
		...
		internal unsafe static uint GetMessageType(SafeCryptMsgHandle safeCryptMsgHandle)
		{
			uint result = 0U;
			uint num = (uint)Marshal.SizeOf(typeof(uint));
			if (!CAPISafe.CryptMsgGetParam(safeCryptMsgHandle, 1U, 0U, new IntPtr((void*)(&result)), new IntPtr((void*)(&num))))
			{
				PkcsUtils.checkErr(Marshal.GetLastWin32Error());
			}
			return result;
		...
		internal static AsnEncodedDataCollection GetAsnEncodedDataCollection(CAPIBase.CRYPT_ATTRIBUTE cryptAttribute)
		{
			...
			{
				IntPtr pBlob = new IntPtr((long)cryptAttribute.rgValue + (long)((ulong)num * (ulong)((long)Marshal.SizeOf(typeof(CAPIBase.CRYPTOAPI_BLOB)))));
		...
		internal unsafe static IntPtr CreateCryptAttributes(CryptographicAttributeObjectCollection attributes)
		{
			...
			IntPtr value = new IntPtr((long)safeLocalAllocHandle.DangerousGetHandle() + (long)((ulong)num2 * (ulong)((long)attributes.Count)));
			foreach (CryptographicAttributeObject cryptographicAttributeObject2 in attributes)
			{
				...
				IntPtr intPtr = new IntPtr(ptr3 + (long)cryptographicAttributeObject2.Values.Count * (long)((ulong)num3) / (long)sizeof(CAPIBase.CRYPTOAPI_BLOB));
				foreach (AsnEncodedData asnEncodedData2 in cryptographicAttributeObject2.Values)
				{
					...
					if (rawData.Length > 0)
					{
						...
						intPtr = new IntPtr((long)intPtr + (long)((ulong)PkcsUtils.AlignedLength((uint)rawData.Length)));
					}
					ptr3++;
				}
				ptr++;
		...
		internal unsafe static CAPIBase.CMSG_SIGNER_ENCODE_INFO CreateSignerEncodeInfo(CmsSigner signer, bool silent)
		{
			...
			result.rgAuthAttr = PkcsUtils.CreateCryptAttributes(signer.SignedAttributes);
			result.cUnauthAttr = (uint)signer.UnsignedAttributes.Count;
			result.rgUnauthAttr = PkcsUtils.CreateCryptAttributes(signer.UnsignedAttributes);
			if (signer.SignerIdentifierType == SubjectIdentifierType.NoSignature)
			{
				...
				result.hCryptProv = invalidHandle.DangerousGetHandle();
				...
				SafeLocalAllocHandle safeLocalAllocHandle = CAPI.LocalAlloc(64U, new IntPtr((long)((ulong)result.SignerId.Value.IssuerSerialNumber.Issuer.cbData)));
				...
				SafeLocalAllocHandle safeLocalAllocHandle2 = CAPI.LocalAlloc(64U, new IntPtr((long)((ulong)result.SignerId.Value.IssuerSerialNumber.SerialNumber.cbData)));
			...
			else
			{
				...
				if (!CAPISafe.CryptAcquireCertificatePrivateKey(certContext, silent ? 70U : 6U, IntPtr.Zero, ref invalidHandle, ref dwKeySpec, ref flag))
				{
					throw new CryptographicException(Marshal.GetLastWin32Error());
				}
				result.dwKeySpec = dwKeySpec;
				...
				if (signer.SignerIdentifierType == SubjectIdentifierType.SubjectKeyIdentifier)
				{
					...
					if (!CAPISafe.CertGetCertificateContextProperty(certContext, 20U, safeLocalAllocHandle3, ref num))
					{
						throw new CryptographicException(Marshal.GetLastWin32Error());
					}
					if (num > 0U)
					{
						...
						result.SignerId.Value.KeyId.cbData = num;
						result.SignerId.Value.KeyId.pbData = safeLocalAllocHandle3.DangerousGetHandle();
