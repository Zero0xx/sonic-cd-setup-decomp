		...
		internal unsafe RecipientInfoCollection(SafeCryptMsgHandle safeCryptMsgHandle)
		{
			...
			for (uint num3 = 0U; num3 < num; num3 += 1U)
			{
				if (flag)
				{
					...
					case 2U:
					{
						...
						case 1U:
						{
							...
							for (uint num5 = 0U; num5 < certIdRecipient.cRecipientEncryptedKeys; num5 += 1U)
							{
								this.m_recipientInfos.Add(new KeyAgreeRecipientInfo(safeLocalAllocHandle, certIdRecipient, num3, num5));
							}
							break;
						}
						case 2U:
						{
							...
							for (uint num6 = 0U; num6 < publicKeyRecipient.cRecipientEncryptedKeys; num6 += 1U)
							{
								this.m_recipientInfos.Add(new KeyAgreeRecipientInfo(safeLocalAllocHandle, publicKeyRecipient, num3, num6));
							}
							break;
						}
						default:
				...
				else
				{
					...
					CAPIBase.CERT_INFO certInfo = (CAPIBase.CERT_INFO)Marshal.PtrToStructure(safeLocalAllocHandle2.DangerousGetHandle(), typeof(CAPIBase.CERT_INFO));
