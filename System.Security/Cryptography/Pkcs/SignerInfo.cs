...
namespace System.Security.Cryptography.Pkcs
{
	...
	public sealed class SignerInfo
	{
		...
		private unsafe void Verify(X509Certificate2Collection extraStore, X509Certificate2 certificate, bool verifySignatureOnly)
		{
			...
			IntPtr value = new IntPtr((long)cert_CONTEXT.pCertInfo + (long)Marshal.OffsetOf(typeof(CAPIBase.CERT_INFO), "SubjectPublicKeyInfo"));
			IntPtr intPtr = new IntPtr((long)value + (long)Marshal.OffsetOf(typeof(CAPIBase.CERT_PUBLIC_KEY_INFO), "Algorithm"));
			IntPtr value2 = new IntPtr((long)intPtr + (long)Marshal.OffsetOf(typeof(CAPIBase.CRYPT_ALGORITHM_IDENTIFIER), "Parameters"));
			IntPtr pvKey = Marshal.ReadIntPtr(intPtr);
			if (CAPI.CryptFindOIDInfo(1U, pvKey, 3U).Algid == 8704U)
			{
				bool flag = false;
				IntPtr ptr = new IntPtr((long)value2 + (long)Marshal.OffsetOf(typeof(CAPIBase.CRYPTOAPI_BLOB), "cbData"));
				IntPtr ptr2 = new IntPtr((long)value2 + (long)Marshal.OffsetOf(typeof(CAPIBase.CRYPTOAPI_BLOB), "pbData"));
			...
			else
			{
				...
				for (;;)
				{
					...
					if (num4 != 0U)
					{
						try
						{
							fixed (byte* ptr4 = this.m_encodedSignerInfo)
							{
								if (!CAPISafe.CryptMsgVerifyCountersignatureEncoded(IntPtr.Zero, 65537U, invalidHandle2.DangerousGetHandle(), num4, new IntPtr((void*)ptr4), (uint)this.m_encodedSignerInfo.Length, cert_CONTEXT.pCertInfo))
								{
				...
				if (string.Compare(crypt_ATTRIBUTE.pszObjId, "1.2.840.113549.1.9.6", StringComparison.OrdinalIgnoreCase) == 0 && crypt_ATTRIBUTE.cValue > 0U)
				{
					if (childIndex < (int)crypt_ATTRIBUTE.cValue)
					{
						...
						if (crypt_ATTRIBUTE.cValue > 1U)
						{
							try
							{
								uint num4 = (uint)((ulong)(crypt_ATTRIBUTE.cValue - 1U) * (ulong)((long)Marshal.SizeOf(typeof(CAPIBase.CRYPTOAPI_BLOB))));
								SafeLocalAllocHandle safeLocalAllocHandle = CAPI.LocalAlloc(64U, new IntPtr((long)((ulong)num4)));
