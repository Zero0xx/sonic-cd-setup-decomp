...
namespace System.Security.Cryptography.Pkcs
{
	...
	public sealed class KeyAgreeRecipientInfo : RecipientInfo
	{
		...
		internal KeyAgreeRecipientInfo(SafeLocalAllocHandle pRecipientInfo, CAPIBase.CMSG_KEY_AGREE_CERT_ID_RECIPIENT_INFO certIdRecipient, uint index, uint subIndex) : base(RecipientInfoType.KeyAgreement, RecipientSubType.CertIdKeyAgreement, pRecipientInfo, certIdRecipient, index)
		{
			IntPtr ptr = Marshal.ReadIntPtr(new IntPtr((long)certIdRecipient.rgpRecipientEncryptedKeys + (long)((ulong)subIndex * (ulong)((long)Marshal.SizeOf(typeof(IntPtr))))));
		...
		internal KeyAgreeRecipientInfo(SafeLocalAllocHandle pRecipientInfo, CAPIBase.CMSG_KEY_AGREE_PUBLIC_KEY_RECIPIENT_INFO publicKeyRecipient, uint index, uint subIndex) : base(RecipientInfoType.KeyAgreement, RecipientSubType.PublicKeyAgreement, pRecipientInfo, publicKeyRecipient, index)
		{
			IntPtr ptr = Marshal.ReadIntPtr(new IntPtr((long)publicKeyRecipient.rgpRecipientEncryptedKeys + (long)((ulong)subIndex * (ulong)((long)Marshal.SizeOf(typeof(IntPtr))))));
			CAPIBase.CMSG_RECIPIENT_ENCRYPTED_KEY_INFO encryptedKeyInfo = (CAPIBase.CMSG_RECIPIENT_ENCRYPTED_KEY_INFO)Marshal.PtrToStructure(ptr, typeof(CAPIBase.CMSG_RECIPIENT_ENCRYPTED_KEY_INFO));
		...
		public CryptographicAttributeObject OtherKeyAttribute
		{
			get
			{
				if (this.m_otherKeyAttribute == null)
				{
					...
					{
						CAPIBase.CRYPT_ATTRIBUTE_TYPE_VALUE cryptAttribute = (CAPIBase.CRYPT_ATTRIBUTE_TYPE_VALUE)Marshal.PtrToStructure(this.m_encryptedKeyInfo.pOtherAttr, typeof(CAPIBase.CRYPT_ATTRIBUTE_TYPE_VALUE));
